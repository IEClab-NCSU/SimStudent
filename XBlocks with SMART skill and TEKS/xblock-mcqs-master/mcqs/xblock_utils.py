...
def util_save_user_activity(state, timestamp, student_module_id, section, subsection, unit, problemId):
    ...
    try:
        ...
        error_flag = True
        print "database rollback and start insert error messages for mcqs..."
        print e
    ...
    try:
        ...
        if not cursor.rowcount:
            ...
            try:
                ...
                if not cursor.rowcount:
                    print "No any other xblocks have the same skill name"
                    setBorderColor = 1
                else:
                    print "Found xblock with the same skill name."
                    setBorderColor = 0
            except Exception as e:

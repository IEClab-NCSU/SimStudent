    ...
    $(function ($) {
        ...
        for(var i = 0; i < unitElements.length; i++) {
            if($(unitElements[i]).attr("data-page-title") == $($(".course_info")[0]).text().split("+")[2].trim()) {
                ...
                console.log("checkbox: " + currentUnitPosition);
            }
        }

        // make sure the following would only works for the student view:
        if(editButton == 0) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        userChoice = data.userChoice == null ? "": data.userChoice.split(",");
...
//                if($(unitElements[position + 1]).attr("data-page-title") == undefined) {
//                    alert("no page-forward event! sent from text");
//                }
//                if($(unitElements[position - 1]).attr("data-page-title") == undefined) {
//                    alert("no page-backward event! sent from text");
//                }

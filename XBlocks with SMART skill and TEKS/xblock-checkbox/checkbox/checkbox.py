""" An XBlock for Checkbox Questions """
...
class CheckboxXBlock(XBlock, StudioEditableXBlockMixin):
    """
    Checkbox Question XBlock
    """
    display_name = String(default='Check List Question', scope=Scope.content)
    block_name = String(default='Check List Question')
    ...
    choices = List(
        ...
        scope=Scope.content, help='Choices for Checkbox Question'
    )
    ...
    # This 'problem_view' attribute is for 'problem_view' column
    ...
    enable_dynamic = String(default="", scope=Scope.user_info)
    enable_hiding = String(default="", scope=Scope.user_info)
    ...
    def insert_user_activities(self, student_module_id, hintCount, clicktype, pageId, section, subsection, unit):
        ...
        if self.user_choice is not None:
            for index, each_choice in enumerate(str(self.user_choice).split(",")):
                if index is not len(str(self.user_choice).split(",")) - 1:
                    userChoice += "check_option_" + each_choice + ","
                    userAnswer += self.choices[int(each_choice) - 1] + ","
                else:
                    userChoice += "check_option_" + each_choice
                    userAnswer += self.choices[int(each_choice) - 1]
        ...
        if self.user_choice is None and clicktype == 'checkbutton':
        ...
        if clicktype == 'checkbutton':
            ...
            question_related = '{"question_details": {"timestamp":"' + timestamp + '", ' \
                               ...
                               '"cf_unit_id":"' + str(self.scope_ids.usage_id.block_id).replace("course", "block") + '", ' \
                               '"cf_action":"checkbox problem hidden",' \
        ...
        util_save_user_activity(state, timestamp, str(student_module_id), section, subsection, unit, problemId)
    ...
    def update_question(self, data, suffix=''):
        ...
        course_id = str(self.scope_ids.usage_id.course_key)
        xblock_id = str(unicode(self.scope_ids.usage_id))
        type_of_xblock = "CheckboxQuestion"
        ...
        self.setBorderColor = util_update_xblock_for_exporter(xblock_id, course_id, self.section, self.subsection, self.unit,
    ...
    def get_default_data(self, data, suffix=''):
        ...
        return {'display_name': self.display_name, 'title': self.title, 'problemId': self.problemId, 'kc': self.kc,
                ...
                'hint': self.hint, 'image_url': self.image_url, 'image_size': self.image_size}
    ...
    def get_pastel_student_id(self, data, suffix=''):
        ...
        return {'pastel_student_id': user_id if self.pastel_student_id is None else self.pastel_student_id,
                'hasBeenSent': self.hasBeenSent, 'enable_dynamic': self.enable_dynamic, 'enable_hiding': self.enable_hiding}

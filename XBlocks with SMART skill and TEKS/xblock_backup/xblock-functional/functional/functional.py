...
class FunctionalXBlock(XBlock, StudioEditableXBlockMixin):
    """
    Functional XBlock
    """
    ...
    def student_view(self, context=None):
        ...
        html = Template(self.resource_string("static/html/functional.html")).render(Context(context))
        frag = Fragment(html)
        frag.add_css(self.resource_string("static/css/functional.css"))
        #frag.add_css(self.resource_string("static/css/font-awesome/css/font-awesome.min.css"))
        frag.add_javascript(self.resource_string("static/js/src/functional.js"))
        frag.initialize_js('FunctionalXBlockInitView')
        """
        frag = Fragment()
    ...
    def studio_view(self, context=None):
        ...
        html = Template(self.resource_string("static/html/functional_edit.html")).render(Context(context))
        frag = Fragment(html)
        frag.add_javascript(self.resource_string("static/js/src/functional_edit.js"))
        frag.initialize_js('FunctionalBlockInitStudio')
        """
        frag = Fragment()
    ...
    def export_course_content(self, data, suffix=''):
        xblock_id = str(unicode(self.scope_ids.usage_id))
        type_of_xblock = "Text Paragraph"
        title = self.text_title
        sub_title = self.text_sub_title
        text = self.text_content
        image_url = self.image_url
        skill_name = self.kc
        db = MySQLdb.connect("127.0.0.1", "root", "", "edxapp_csmh");
        cursor = db.cursor();
        sql = """INSERT INTO edxapp_csmh.export_course_content(xblock_id, type_of_xblock, title, sub_title, text, image_url, skillname) VALUES (%s, %s, %s, %s, %s, %s, %s)"""
        
        try:
            cursor.execute(sql, (xblock_id, type_of_xblock, title, sub_title, text, image_url, skill_name))
            db.commit()
        except Exception as e:
            print e
            db.rollback()

...
import datetime
import time
# read problem name from brd file
...
class CTATXBlock(XBlock):
    ...
    def ctat_log(self, data, dummy_suffix=''):
	    ...
	    info = logdata['message'][logdata['message'].index("%3C%3Fxml"):logdata['message'].index("</log_action>")]
	    info = urllib2.unquote(info)
	    attempt = False
	    if info[info.index("name=")+6:info.index("name=")+13]=="ATTEMPT":
	    	selectionString = info[info.index("<selection>")+11:info.index("</selection>")]
	    	actionString = info[info.index("<action>")+8:info.index("</action>")]
	    	inputString = info[info.index("<input>")+7:info.index("</input>")]
	    	#inputString = inputString[9:inputString.index("]]")]
	    	attempt = True
	        print info
	        print "Selection",selectionString,"Action",actionString,"Input",inputString
        ...
        # pylint: enable=broad-except
	if attempt:
            return {'result': 'success', 'selection': selectionString, 'action': actionString, 'input': inputString}
	else:
	    return {'result': 'successful'}

    #### Logging function starts from here: geet_student_id:
    @XBlock.json_handler
    def get_student_id(self, data, suffix=''):
        ...
        input_value = str(data.get('input'))
        ...
        try:
            ...
            self.get_module_id(str(row[3]), xblock_id, hintCount, str(data.get('type')), correctness, kc, selection, action, input_value)
        
        return {'user': 'something'} 
    
    
    #This method help us to find module_id for our XBlock
    def get_module_id(self, student_id, xblock_id, hintCount, clicktype, correctness, kc, selection, action, input_value):
        print '*' + student_id + '*'
        print '*' + xblock_id + '*'
        print '*' + clicktype + '*'
        try:
            ...
            if student_module_id is None:
                print "Student_Module_Id is none..."
            else:
        ...
        if student_module_id is None:
            self.insert_user_activities('999', hintCount, clicktype, correctness, kc, selection, action, input_value)
        else:
            self.insert_user_activities(student_module_id[0], hintCount, clicktype, correctness, kc, selection, action, input_value)
    
    #This method help us to store the user activities in OpenEdx database:
    def insert_user_activities(self, student_module_id, hintCount, clicktype, correctness, kc, selection, action, input_value):
        # first, collect all the information
        correct_map = '"correct_map":{"ctat_id":{"hint":"'+ "N/A" + '", "hintmode": "click", "correctness":"'+ correctness + '", "msg": "'+ "N/A" + '", "answervarible": null, "npoints": null, "queuestate": null}}'
        input_state = '"cata_input_state": {}'
        ts = time.time()
        timestamp = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        ...
        student_answers = '"student_answers":{"ctat_answers":"' + "N/A" + '"}'
        
        #get problem name from brd file or html file:
        
        #problem_name = ((self.brd).split('/')[len(self.brd.split('/'))- 1]).split('.')[0]
        #print problem_name
        # download the BRD file then read it:===============================
        print "Begin download the file with urllib2"
        f = urllib2.urlopen(self.brd)
        with open("CTATXblock-v2/ctatxblock/problem.xml", "wb") as code :
            code.write(f.read())
        print "End with download the file..."
        # start to read this xml file to get problem name:
        dom = xml.dom.minidom.parse('problem.xml')
        root = dom.documentElement
        problem_name = root.getElementsByTagName('ProblemName')[0].firstChild.data
        print problem_name
        # ==================================================================
        
        if clicktype == 'checkbutton':
            question_related = '"question_details":{"display_name":"' + "CTAT XBlock" + '", "problem name":"' + problem_name + '", "problemId": "' + "N/A" + '", "question": "' + "N/A" + '", "user_input": "' + input_value + '", "kc": "' + kc + '", "time zone": "US/Central", "student response type": "ATTEMPT", "student response subtype": "N/A", "tutor response type": "RESULT","tutor response subtype": "N/A", "level": "N/A", "problem view": "1", "step name": "N/A", "attemp at step": "' + "N/A" + '", "selection": "' + selection + '", "Action": "' + action + '", "input": "' + input_value + '", "feedback text": "' + "N/A" + '", "feedback classification": "N/A", "help level": "' + "N/A" + '", "total number hints": "' + "N/A" + '", "condition name": "N/A", "condition type": "N/A", "kc category": "N/A", "school": "N/A", "class": "N/A", "cf": "N/A"}'
        else:

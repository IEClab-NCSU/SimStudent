...
class TextXBlock(XBlock, StudioEditableXBlockMixin):
    ...
    def update_question(self, data, suffix=''):
        ...
        util_update_xblock_for_exporter(xblock_id, course_id, self.section, self.subsection, self.unit, type_of_xblock, title, sub_title, text, image_url, skillname)
    ...
    def get_default_data(self, data, suffix=''):
        """
        when text_edit page is on load, get all the default data from here
        """
        SMARTskillset = util_get_SMARTskillset(str(unicode(self.scope_ids.usage_id)))
        print SMARTskillset
        print "hello-----------------"
        skillset = util_get_skillset(str(self.scope_ids.usage_id.course_key))
        return {'display_name': self.display_name, 'title': self.title, 'SMARTkc': SMARTskillset, 'kc': self.kc, 'text_title': self.text_title, 'text_content': self.text_content, 'text_sub_title': self.text_sub_title, 'image_url': self.image_url, 'image_size': self.image_size, "skillset": skillset}
    ...
    def get_course_info(self, data, suffix=''):
        return {"section": self.section, "subsection": self.subsection, "unit": self.unit, "preUnit": self.preUnit,
                "nextUnit": self.nextUnit}

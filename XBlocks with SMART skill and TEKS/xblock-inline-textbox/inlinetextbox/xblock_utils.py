...
def util_save_user_activity(state, timestamp, student_module_id, section, subsection, unit, problemId):
    ...
    try:
        ...
        error_flag = True
        print "database rollback and start insert error messages for inline textbox..."
        print e
...
def util_update_xblock_for_exporter(xblock_id, course_id, section, subsection, unit, type_of_xblock, title, question,image_url, correct_answer, hint, problem_name, skillname):
    ...
    try:
        ...
        if not cursor.rowcount:
            print "No any results(assessment) found, insert a new entry to assessment for inlinetextbox xblock:"
            try:
                cursor.execute(sql, (
...
def util_save_module_skillname(skillname, xblock_id, location_id):
    ...
    try:
        ...
        result = cursor.fetchone()
        if not cursor.rowcount:
            cursor.execute(sql1, (xblock_id, "inlinetextbox", skillname, location_id))
            db.commit()
            print "Skillname has been saved in module_skillname table."
        else:
            cursor.execute(sql2, ("inlinetextbox", skillname, location_id, xblock_id))
            db.commit()
    except Exception as e:
        print e
        db.rollback()
...
def util_get_border_color(course_id, skillname):
    ...
    try:
        ...
        result1 = cursor.fetchone()
        if not cursor.rowcount:
            print "No any other xblocks have the same skillname - inlinetextbox."
            HtmlSetBorderColor = 0
        else:
            print "Found xblock with the same skillname - inlinetextbox."

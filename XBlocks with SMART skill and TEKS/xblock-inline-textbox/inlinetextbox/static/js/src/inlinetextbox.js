...
function InlineTextboxXBlockInitView(runtime, element) {
    ...
    $(function ($) {
        ...
        if(editButton == 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    userAnswer = data.userAnswer.split("|");

                    var inputBox = $("div[data-id='" + xblock_id + "'] input[class='userAnswer']");

                    for(var i = 0; i < userAnswer.length; i++) {
                        $(inputBox[i]).val(userAnswer[i]);
                    }
                    ...
                    if(countTimes >= 1) {
                        if(correctOrNot) {
                            ...
                            $("div[data-id='" + xblock_id + "'] div[id='answers'] span").remove();
                            $("div[data-id='" + xblock_id + "'] div[id='answers'] br").remove();

                            var correctMessage = "<i id='tick' class='tick'>&nbsp;&nbsp;&nbsp;  &#x2713; &nbsp;&nbsp;&nbsp;Correct</i>";

                            for(var i = 0; i < userAnswer.length; i++) {
                                $("div[data-id='" + xblock_id + "'] div[id='answers']").append("<span>(" + (i + 1) + ")." + correctMessage + "</span><br/>");
                                $($("div[data-id='" + xblock_id + "'] input[class='userAnswer']")[i]).css("border-color", "green");
                            }
                            ...
                            $("div[data-id='" + xblock_id + "'] div[id='answers'] br").remove();
                            var correctMessage = "<i id='tick' class='tick'>&nbsp;&nbsp;&nbsp;  &#x2713; &nbsp;&nbsp;&nbsp;Correct</i>";
                            var incorrectMessage = "<i id='ballot' class='ballot'>&nbsp;&nbsp;&nbsp; &#10006; &nbsp;&nbsp;&nbsp; Incorrect, please try it again.</i>";
                            var correctAnswers = data.correct_answer.split("|");
                            for(var i = 0; i < correctAnswers.length; i++) {
                                if(correctAnswers[i].toLowerCase() == userAnswer[i].toLowerCase()) {
                                    $("div[data-id='" + xblock_id + "'] div[id='answers']").append("<span>(" + (i + 1) + ")." + correctMessage + "</span><br/>");
                                    $($("div[data-id='" + xblock_id + "'] input[class='userAnswer']")[i]).css("border-color", "green");
                                } else {
                                    $("div[data-id='" + xblock_id + "'] div[id='answers']").append("<span>(" + (i + 1) +  ")." + incorrectMessage + "</span><br/>");
                                    $($("div[data-id='" + xblock_id + "'] input[class='userAnswer']")[i]).css("border-color", "red");
                                }
        ...
        */
        ...
        $("#getHint", element).on('click', function(e){
            ...
            $.ajax({
                url: runtime.handlerUrl(element, "increase_hint_numbers"),
                type: "POST",
                data: JSON.stringify({"increase_hint": 1}),
                success: function() {}
            });
    ...
    function checkAnswer() {
        var selectedCheck = $("div[data-id='" + xblock_id + "'] input[class='userAnswer']");
        var user_answers = "";
        for(var i = 0; i < selectedCheck.length; i++) {
            if(i != selectedCheck.length - 1) {
                user_answers += $(selectedCheck[i]).val() + "|"
            } else {
                user_answers += $(selectedCheck[i]).val();
            }
        }
        ...
        $.ajax({
            type: "POST",
            data: JSON.stringify({'setStatus': true, 'userAnswer': user_answers, 'hasBeenSent': 'true'}),
            url: setStatusWhenRefresh,
            async: false
        });
        
        var answer = user_answers;
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data.correct == true){
                    ...
                    $("div[data-id='" + xblock_id + "'] div[id='answers'] br").remove();
                    var correctMessage = "<i id='tick' class='tick'>&nbsp;&nbsp;&nbsp;  &#x2713; &nbsp;&nbsp;&nbsp;Correct</i>";
                    var answer_boxes = userSelected.split("|");
                    ...
                    for(var i = 0; i < answer_boxes.length; i++) {
                        if(correctAnswers[i].toLowerCase() == answer_boxes[i].toLowerCase()) {
                            $("div[data-id='" + xblock_id + "'] div[id='answers']").append("<span>(" + (i + 1) + ")." + correctMessage + "</span><br/>");
                            $($("div[data-id='" + xblock_id + "'] input[class='userAnswer']")[i]).css("border-color", "green");
                        } else {
                            $("div[data-id='" + xblock_id + "'] div[id='answers']").append("<span>(" + (i + 1) +  ")." + incorrectMessage + "</span><br/>");

...
/**
 * The Combinations class provides an enumeration of all subsets of a
 * group of n objects taken r at a time. The constructor for
 * Combinations accepts the group as an array of Objects, along with
...
public class Combinations implements java.util.Enumeration {
    ...
    public Combinations(Object[] inArray, int m) throws CombinatoricException {
    	 ...
    	 * combination to return.  For example, an index on 5 things
    	 * taken 3 at a time might contain {0 3 4}.  This index will
    	 * be followed by {1 2 3}. Initially, the index is {0 ... m -
    	 * 1}.
    	 */
    	index = new int[m];
    	for (int i = 0; i < m; i++)
     ...
     * rightmost index element that can be incremented, increments it,
     * and then changes the elements to the right to each be 1 plus
     * the element on their left.
     * <p>
     *
     * For example, if an index of 5 things taken 3 at a time is at {0
     * 3 4}, only the 0 can be incremented without running out of
     * room. The next index is {1, 1+1, 1+2) or {1, 2, 3}. This will
     * be followed by {1, 2, 4}, {1, 3, 4}, and {2, 3, 4}.
     * <p>
     ...
     * say just Object, because the Combinations class implements
     * Enumeration, which declares that the nextElement() returns a
     * plain Object. Users must cast the returned object to
     * (Object[]).
     */

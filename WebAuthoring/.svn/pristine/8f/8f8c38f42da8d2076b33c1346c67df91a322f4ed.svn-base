...
CTATMatcherComparator = function()
{
	...
	CTATMatcherComparator.compareBasedOnType = function(object1, object2)
	{
		...
		for(var i in matcherPrecedenceOrder)
		{
			if(matcherPrecedenceOrder.hasOwnProperty(i))
			{
				if(object1 instanceof matcherPrecedenceOrder[i])
				{
					break;
				}
			}
		...
		for(var j in matcherPrecedenceOrder)
		{
			if(matcherPrecedenceOrder.hasOwnProperty(j))
			{
				if(object2 instanceof matcherPrecedenceOrder[j])
				{
					break;
				}
			}

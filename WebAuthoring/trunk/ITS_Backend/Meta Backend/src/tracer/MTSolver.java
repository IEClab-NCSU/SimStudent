...
public class MTSolver {
	...
	private Rete engine;
	private TraceInputFunction traceFunction;
	...
	private void addUserFunctions() {
		...
		this.traceFunction = new TraceInputFunction(this);
	...
	public MTResult sendSAI(MTSAI sai) throws MTException, JessException {
		...
		while(it.hasNext()) {
			try {
				int numberOfRulesActuallyFired=this.getEngine().run(1);
				// if here then student input is correct
//				if (logProductionRules)
//					MTLog("Fired production rule: " + it.next());
				
				String rule = (String) it.next();								//CASTING FIRED RULE
				this.hintFunction.resetHint();
				finishSending();
				return new MTResult(MTResultType.CORRECT, sai.toString(), rule);  //MADE THIS CHANGE
			} catch (MTException e) {
...
//							MTLog("Matched buggy production rule: " + it.next());
						String rule = (String) it.next();							//CASTING FIRED RULE
						finishSending();
						return new MTResult(MTResultType.BUGGY, e.getMessage(), rule);
					default:
	 ...
	 * @throws MTException
	 */
	public String getHint() throws JessException, MTException {
		if (!prepareHint()) {
			return "";
		}
		Iterator it = this.engine.listActivations();

...
CTATMessage = function(aMessage)
{
	...
	//this.ctatdebug (messageParser.toJSONString (aMessage.value));

	var gradeResult="ungraded";
	var isLogMessage=false;
	var hassai=false;
	var rawMessage="";
	var skills=new CTATSkillSet ();
			...
			for (var t=0;t<tList.length;t++)
			{
				...
				if (messageParser.getElementName (entry)=="properties")
				{
					...
					for (var w=0;w<aList.length;w++)
					{
						...
						this.ctatdebug ("Nodename: " + messageParser.getElementName (test));

						if (messageParser.getElementName (test)=="Skills")
						{
							skills.parseByValue (test);
						}

						if (messageParser.getElementName (test)=="SuccessMsg")
						{
							successMsg=selection=messageParser.getNodeTextValue (test);
						}

						if (messageParser.getElementName (test)=="BuggyMsg")
						{
							buggyMsg=messageParser.getNodeTextValue (test);
						}

						if (messageParser.getElementName (test)=="Selection")
						{
							selection=messageParser.getNodeTextValue (test);
						}

						if (messageParser.getElementName (test)=="Action")
						{
							action=messageParser.getNodeTextValue (test);
						}

						if (messageParser.getElementName (test)=="Input")
						{
							input=messageParser.getNodeTextValue (test);
						}

						if (messageParser.getElementName (test)=="prompt")
						{
		...
		else
		{
			...
			*/
			...
			for (var t=0;t<tList.length;t++)
			{
				...
				if (messageParser.getElementName (entry)=="event_descriptor")
				{
					var aList=messageParser.getElementChildren (entry);

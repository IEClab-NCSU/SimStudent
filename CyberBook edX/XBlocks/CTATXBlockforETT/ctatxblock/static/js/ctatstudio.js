...
function CTATXBlockStudio(runtime, element)
{
    ...
    $(element).find('.save-button').bind('click', function() {
        /*
        var updateProblemUrl = runtime.handlerUrl(element, 'update_problem_name');
        $.ajax({
            url: updateProblemUrl,
            type: "POST",
            data: JSON.stringify({'problemName': $(element).find('input#problemName').val(), 'title': $(element).find('input#title').val()}),
            async: false
        });
        */
        ...
        var data = {
            ...
            title: $(element).find('input#title').val(),
            problem: $(element).find('input#problemName').val()
        };
        ...
        */
        ...
        $.ajax({
            ...
            success: function(response) {
                //alert("I am finish and come back");
                if (response['result'] != 'success') {
                    console.log("response object: " + response);
                    if (response['error']) {
                        alert(response['error']);

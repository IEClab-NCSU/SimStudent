...
public class computeKTparamsAll {
    ...
    private String students_[];// Number of instances
    private String skill_[];
    private double right_[];
    private int skillends_[];//Number of Skills
    private int skillnum;
    private boolean lnminus1_estimation;
    private boolean bounded;
    private boolean L0Tbounded;

    public computeKTparamsAll() {
        this.students_ = new String[27600];
        this.skill_  = new String[27600];
        this.right_ = new double[27600];
        this.skillends_ =  new int[10];
        this.skillnum = -1;
    ...
    public void read_in_data(StreamTokenizer st_) {
        ...
        try {
            ...
            while (tt != StreamTokenizer.TT_EOF) {
                ...
                //System.out.println("Lesson: " + tt);

                tt = st_.nextToken();
                students_[actnum] = st_.sval;
                //System.out.println("student: " + tt);

                tt = st_.nextToken();
                skill_[actnum] = st_.sval;
                //System.out.println("skill: " + tt);
                ...
                actnum++;
                //System.out.println("skill_: " + skill_[actnum - 1]);
    ...
    public double[] findGOOF(int start, int end, double Lzero, double trans,
        ...
        for (int i = start; i <= end; i++) {
           ...
           // System.out.println("students_[i] object is null? " + students_[i] == null);
            ...
            if (lnminus1_estimation)
                ...
                likelihoodcorrect = (prevL * (1.0 - S)) + ((1.0 - prevL) * G);  // correctness
            SSR += (right_[i] - likelihoodcorrect) * (right_[i] - likelihoodcorrect);

            if (right_[i] == 1.0)
                prevLgivenresult = ((prevL * (1.0 - S)) / ((prevL * (1 - S)) + ((1.0 - prevL) * (G)))); // P(L0|Correct)
            else
                prevLgivenresult = ((prevL * (S)) / ((prevL * (S)) + ((1.0 - prevL) * (1.0 - G)))); // P(L0|Incorrect)
        ...
        dou[0] = SSR;
        dou[1] = prevLgivenresult;
        return dou;
    }

    public double computelzerot(String infile_) {
        ...
        for (int curskill = 0; curskill <= skillnum; curskill++) {
            probability = fit_skill_model(curskill);
        }
        return probability;
    }

    public double fit_skill_model(int curskill) {
        double SSR = 0.0;
            ...
            for (double trans = 0.01; trans <= topT; trans = trans + 0.01) {
                for (double G = 0.01; G <= topG; G = G + 0.01) {
                    for (double S = 0.01; S <= topS; S = S + 0.01) {
                        SSR = findGOOF(startact, endact, Lzero, trans, G, S)[0];
                        prevLgivenresult = findGOOF(startact, endact, Lzero, trans, G, S)[1];
                        /**
            ...
            for (double G = startG - 0.009; ((G <= startG + 0.009) && (G <= topG)); G = G + 0.001) {
                for (double S = startS - 0.009; ((S <= startS + 0.009) && (S <= topS)); S = S + 0.001) {
                    for (double trans = starttrans - 0.009; ((trans <= starttrans + 0.009) && (trans < topT)); trans = trans + 0.001) {
                        SSR = findGOOF(startact, endact, Lzero, trans, G, S)[0];
                        prevLgivenresult = findGOOF(startact, endact, Lzero, trans, G, S)[1];
                        //System.out.println("When the student did correctly, how much probability of the student understand the knowledge: " + SSR);
                        if (SSR < BestSSR) {
                            BestSSR = SSR;
                            bestLzero = Lzero;
                            besttrans = trans;
        ...
        //System.out.println("Probability of the student understand the knowledge: " + prevLgivenresult);
        return prevLgivenresult;
    }


    public static void main(String args[]) {
        String infile_ = "StudentData/1a2b3c_average_speed.txt";//Needs to be tab delimited
        computeKTparamsAll m = new computeKTparamsAll();
        double probability = m.computelzerot(infile_);
        System.out.println(probability + ", this is the probability that I get.");
    }

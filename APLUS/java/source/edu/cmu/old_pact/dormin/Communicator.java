...
//		trace.out("sending error message for "+moString);
		MessageObject mo = new MessageObject("NoteError");
		int messageNum = -1;
		Integer parsedNum = (Integer)(msgParsed.getOptionalParameter("MESSAGENUMBER"));
		if (parsedNum == null)
		...
		//e.printStackTrace();
		mo.addParameter("OBJECT",topProxy);
		mo.addParameter("InResponseTo", messageNum);
		String errorClass = e.getClass().getName();
		if ((errorClass.toUpperCase()).startsWith("DORMIN."))
			errorClass = errorClass.substring(7);
		mo.addParameter("ErrorClass", errorClass);
		mo.addParameter("ErrorText", e.getMessage()); 
		if(doSendError)
			topProxy.send(mo);	
	...
	protected boolean isIgnorable(MessageObject mo, String exceptionName){
		try{
			Vector ignore = mo.extractListValue("IGNOREERRORCLASSES");
			...
			String inVector = "";
			for(int i=0; i<s; i++){
				inVector = "DORMIN."+(String)ignore.elementAt(i);
...
//			topProxy.mailToProxy(inEvent, allObjDesc);
			Object objParam = inEvent.getParameter("OBJECT");
			String verb = inEvent.extractVerb();
			trace.out("mt", "VERB = " + verb);
			if (objParam == null) {
				trace.err("Error: missing object parameter: " + inEvent.getParameterNames());
				sendErrorMessage(new MissingParameterException("Object",inEvent.getParameterNames()),inEvent,inEvent.toString());
			} else if (verb == null) {
				trace.out (10, this, "Error: missing verb parameter");
				sendErrorMessage(new MissingParameterException("Verb",inEvent.getParameterNames()),inEvent,inEvent.toString());
			} else  {
		...
		//} catch (MissingParameterException ex) {
			...
			trace.out (5, this, "Dormin exception e = " + e);

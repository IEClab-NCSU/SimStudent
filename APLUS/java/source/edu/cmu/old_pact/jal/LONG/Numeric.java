...
public final class Numeric
{
   ...
   */
  public static long accumulate(long[] array, int first, int last,
				  long init)
    {
      long acc = init;
      while (first < last)
	acc = (long)(acc + array[first++]);
      return acc;
    }
   ...
   */
  public static long inner_product(long[] array1, long[] array2,
				     int first1, int last1, int first2,
				     long init)
    {
      long acc = init;
      while (first1 < last1)
	acc = (long) (acc + array1[first1++] * array2[first2++]);
      return acc;
    }
   ...
   */
  public static int partial_sum(long[] source, long[] dest,
   ...
   *                  <code>to + (last - first)</code>.
   */
  public static int adjacent_difference(long[] source, long[] dest,
					int first, int last, int to)
    {
      if (first < last) {
	dest[to] = source[first];

	long prev_value = source[first];
	while (++first < last) {
	  long cur_value = source[first];
	  dest[++to] = (long) (cur_value - prev_value);

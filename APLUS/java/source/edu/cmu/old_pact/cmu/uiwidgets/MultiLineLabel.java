...
public class MultiLineLabel extends Canvas {
  ...
  public MultiLineLabel(String label, int margin_width,
                        int margin_height, int alignment, int initLength ) {
    this.label = label;                 // Remember all the properties.
    this.margin_width = margin_width;
    this.margin_height = margin_height;
    this.alignment = alignment;
    this.lineLength = initLength;
    newLabel();                         // Break the label up into lines.
  }
  public MultiLineLabel(String label, int margin_width, int margin_height) {
    this(label, margin_width, margin_height, LEFT, 80);
  }
  public MultiLineLabel(String label, int alignment) {
    this(label, 10, 10, alignment, 80);
  }
  public MultiLineLabel(String label) { this(label, 10, 10, LEFT, 80); }
  public MultiLineLabel(int len,String label) { this(label, 10, 10, LEFT, len); }
  ...
  // added two functions, so this class can be used to get multiLines  
  public void  setLineLength(int len) {
  	lineLength = len;
  }
  public Vector getMultiLines(String forlabels) {
  	setLabel(forlabels);
  	Vector toret = new Vector();
  	for(int i=0; i<num_lines; i++)
  		toret.addElement(lines[i]);
  	return toret;
  }
  
  public void createMulti() {
  	 int currPos, len, linelen;
  	 String line = label, remain, buf;
     len = line.length();
     currPos = 0;
     linelen = lineLength;
     
     sub:while (currPos < len)  {
     	if(len <= (currPos+linelen))  {
          	line.concat("\n");
          	currPos = len;
     	}else if  (len > (currPos+linelen)) {
          	if(line.charAt(currPos+linelen) != ' ')  {
          		int i;
          		i = line.lastIndexOf(' ', (currPos+linelen));
          		remain = line.substring(0, i) + "\n";
          		buf = line.substring((i+1), len);
          		line = remain + buf;
          		currPos = i + 1;
     		}else if(line.charAt(currPos+linelen) == ' ')  {
          		remain = line.substring(0, currPos+linelen) + "\n";
          		buf = line.substring(((currPos+linelen)+1), len);
          		line = remain + buf;
            	currPos = currPos + linelen + 1;
        	}
         }
     }
     label = line;

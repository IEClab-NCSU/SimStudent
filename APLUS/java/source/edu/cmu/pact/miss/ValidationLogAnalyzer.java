...
public class ValidationLogAnalyzer {
	...
	public void addTally(Validation validation) {
	    ...
	    // trace.out("addTally for " + validation.fitType());
	...
	if (args.length < 1 || 2 < args.length) {
	    ...
	    trace.out("Usage: ValidationLogAnalyzer <log_file> {<cut_off>}");
	    trace.out("<log_file> must be the validation log file generated by SimSt.");
	    trace.out("An optional <cut_off> specifies # of problems for pre-learning.");
	}
	...
	while ((record = in.readLine()) != null) {
	    ...
	    // trace.out("validation: " + validation);
		...
		// trace.out("CT added. Total " + getCT().size() + " tables.");
	    }

 ...
 * 
 */
public class GetOptimalSteps implements Userfunction, Serializable {

	/**	 */
	private static final long serialVersionUID = 1L;

	/**	 */
	private static final String GET_OPTIMAL_STEPS = "get-optimal-steps";
	 ...
	 * @param controller
	 */
	public GetOptimalSteps(ModelTracer amt) {
		this.amt = amt;
	}
	...
	public Value call(ValueVector vv, Context context) throws JessException {
		if(!vv.get(0).stringValue(context).equals(GET_OPTIMAL_STEPS)) {
			throw new JessException(GET_OPTIMAL_STEPS, "called but ValueVector head differs", vv.get(0).stringValue(context));
		}
		...
		if(problem.length() > 0 && problem != SimStRete.NOT_SPECIFIED) {	
			...
			int steps=solver.getSolutionStepInfo().size();
			steps=steps+2; //to account for done;
			...
			Value rtnValue = new Value(steps, RU.INTEGER);

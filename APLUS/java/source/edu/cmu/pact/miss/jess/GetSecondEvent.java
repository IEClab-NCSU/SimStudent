...
public class GetSecondEvent implements Userfunction, Serializable {
	...
	public GetSecondEvent(ModelTracer amt) {
		this.amt = amt;
	}
	...
	public Value call(ValueVector vv, Context context) throws JessException {
		...
		if(!vv.get(0).stringValue(context).equals(GET_SECOND_EVENT)) {
			throw new JessException(GET_SECOND_EVENT, "called but ValueVector head differs", vv.get(0).stringValue(context));
		}
		...
		if(events != null && events.size() > 1 && context.getEngine() instanceof SimStRete) {
			
			rtnValue = (String) events.get(1);

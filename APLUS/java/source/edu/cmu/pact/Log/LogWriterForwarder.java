 ...
 */
public class LogWriterForwarder extends LinkedList<String> implements Runnable {
	...
	protected String log(String logMsg) {
		...
		if (trace.getDebugCode("logservice")) trace.out("logservice", "log("+logMsg+")");
		synchronized(writer) {
			try {
				writer.write(logMsg.trim());   // trim: no trailing newline
				writer.flush();
	 ...
	 * @param logMsg string to log
	 */
	public String logOrQueueAndReply(String logMsg) {
		String response = log(logMsg);       // returns null if shouldn't log
		if(!canForward())
			return STATUS_FAILURE+"cause=service_unavailable\r\n";
		int queueLength = enqueue(logMsg);
		if(response == null)
			response = STATUS_SUCCESS+"queue_position="+queueLength+"\r\n";
		else

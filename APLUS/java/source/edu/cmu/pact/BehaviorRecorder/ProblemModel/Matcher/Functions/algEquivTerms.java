 ...
 * their differences are limited to order of commutative operands, redundant
 * parentheses and notational variants (2*x and 2x).  
 */
public class algEquivTerms implements UsesVariableTable, Serializable
{
	 ...
	 */	
	public Boolean algEquivTerms(String expression1 ,String expression2)
	{
		CTATExpressionParser matcher = new CTATExpressionParser(properties);
		//Double result = matcher.evaluate(expression);
		Boolean result = matcher.twoExpressionsEqualBasic(expression1, expression2);
	...
	public static void main(String[] args) {
		algEquivTerms fn = new algEquivTerms();
		...
		for(int i = 1; i < args.length; ++i) {
			try {
				System.out.printf("%s %s %s\n",
						args[0], (fn.algEquivTerms(args[0], args[i]) ? "==" : "!="), args[i]);
			} catch (Exception e) {

 ...
 * their differences are limited to order of commutative operands.
 ...
 */
public class algStrictEquivTerms implements UsesVariableTable, Serializable
{
	 ...
	 */	
	public Boolean algStrictEquivTerms(String expression1, String expression2)
	{
		CTATExpressionParser matcher = new CTATExpressionParser(properties);
		String s1 = matcher.strictBasicTerms(expression1);
		String s2 = matcher.strictBasicTerms(expression2);
		Boolean result = null;
		if(s1 == null)
	...
	public static void main(String[] args) {
		algStrictEquivTerms fn = new algStrictEquivTerms();
		...
		for(int i = 1; i < args.length; ++i) {
			try {
				System.out.printf("%s %s %s\n",
						args[0], (fn.algStrictEquivTerms(args[0], args[i]) ? "==" : "!="), args[i]);
			} catch (Exception e) {

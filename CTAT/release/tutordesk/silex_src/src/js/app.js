...
goog.require('silex.view.ComponentPalette');
...
goog.require('silex.SilexRightClickMenu');

const SERVER_ROOT="http://augustus.pslc.cs.cmu.edu/tutordesk/"
...
class App {
  ...
  constructor() {
	...
	this.view.componentPalette.buildUi();
    ...
    console.log("Silex redraw workspace");
	this.view.workspace.redraw(this.view);

	this.rightClickMenu = new silexRightClickMenu();
	...
	this.controller.fileMenuController.initSaveTimer();
	
	console.log("End silex.app initialization");
  }

  /**
  *	Init the context menu event listener on the workspace
  */
  setRightClickListener()
  {
	  this.rightClickMenu.resetHandler();
  }
   ...
   */
  initView() {
    console.log("Silex.app -- initView()");
	// Stage
    ...
    var menu = new silex.view.Menu(menuElement, this.model, this.controller, this.view);
    
	// context menu
    var contextMenuElement = /** @type {!Element} */ (goog.dom.getElementByClass('silex-context-menu'));
    /** @type {silex.view.ContextMenu} */
    var contextMenu = new silex.view.ContextMenu(contextMenuElement, this.model, this.controller, this.view);
	...
	//palette
	var componentPalette = new silex.view.ComponentPalette(this.model, this.controller);
	...
	console.log("End silex.App() -- initView()");
  }
   ...
   */
  initModel() {
    console.log("Silex.app() -- initModel()");
   ...
   */
  initController() {
	console.log("Silex.app() -- initController()");
    this.controller.init(
		...
		new silex.controller.ComponentPaletteController(this.model, this.view)
    );
  }
}

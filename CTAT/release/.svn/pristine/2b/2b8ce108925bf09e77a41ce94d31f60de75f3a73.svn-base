...
silex.view.pane.PaneBase = function(element, model, controller) {
  ...
  this.minBtn = element.querySelector('.prop-container-collapse');
  this.container = element.querySelector('.pane-container');
  this.minBtn.addEventListener('click', this.toggleVisible.bind(this));
};
...
silex.view.pane.PaneBase.prototype.toggleVisible = function()
{
	if (this.visible)
	{
		this.container.classList.add('hidden');
		this.minBtn.innerHTML = '+';
		this.minBtn.setAttribute('title', this.minBtn.getAttribute('title').replace('collapse', 'expand'));
	}
	else
	{
		this.container.classList.remove('hidden');
		this.minBtn.innerHTML = '-';
		this.minBtn.setAttribute('title', this.minBtn.getAttribute('title').replace('expand', 'collapse'));
	}
 ...
 */
silex.view.pane.PaneBase.prototype.styleChanged = function(styleName, opt_styleValue, opt_elements, opt_undoable) {
  //  if (this.iAmRedrawing) return;
  this.iAmSettingValue = true;
  // notify the controller
  this.controller.propertyToolController.styleChanged(styleName, opt_styleValue, opt_elements, opt_undoable);
  this.controller.contextMenuController.view.contextMenu.redraw();
...
/**
*	Get a list of which panes should be enabled based on selected component(s)
*	@param selectedElements the currently selected elements on the stage
*/
silex.view.pane.PaneBase.prototype.getDisabled = function(selectedElements)
{
	var disable = silex.Config.UIDisabled['default'];
	for (let i = 0; i < selectedElements.length; i++) //for each selected
	{  
		//TODO: optimize this by selecting based on one specific attr only, rather
		//		than having to iterate through the whole classlist
		for (let j = 0; j < selectedElements[i].classList.length; j++) //for each class
		{
			let list = silex.Config.UIDisabled[selectedElements[i].classList.item(j)];
			if (list)
			{
				//take intersection of list and disable
				disable = disable.filter(function(value)
					{
						return (list.indexOf(value) > -1);
					});
				break;
			}
			else if (j == selectedElements[i].classList.length-1) 
			{
				//at end of classList and still no match
				//means we found an element w/ no entry
				disable = [];
				i = selectedElements.length; //break outer loop
				break;
			}
		}
	}
...
silex.view.pane.PaneBase.prototype.convertToInt = function(value)
{
	if (value)
	{
		var len = value.indexOf('px');
		if (len > -1)

...
* save as
*/
function file_saveas(callback, filename, filetext, fileparentID, filetype){
  ctatdebug("file_saveas");
  var save=function(name,text,parentID,type){
    if(parentId){
      drive.insertFile(name,text,type,parentID,function(file){
        currentFileId=file.id;
        callback();
      });
    }
    else{
      drive.insertFile(name,text,type,rootFolder,function(file){
        currentFileId=file.id;
        if(callback){
          callback();
        }
  ...
  var fun = function(){
    if (!filename) filename = "some_name.txt";
    if (!filetext) filetext = "some text";
    /*some stuff checking if user wants to save in different folder */
    var useRootFolder = !fileparentID;//value depends on above
    
    if(useRootFolder){//user doesn't opt to change folders
      save(filename,filetext,false,filetype);
    }else{//user opts to change folders
      drive.pickFolder(true,function(fileparentID){
        ctatdebug("id: "+fileparentID);
        save(filename,filetext,fileparentID,filetype);
      });
    }
  }

/**-----------------------------------------------------------------------------
 ...
 $HeadURL: svn://pact-cvs.pact.cs.cmu.edu/usr5/local/svnroot/AuthoringTools/branches/CTAT_4_2_Release/HTML5/src/CTATComponents/CTATScrollPaneComponent.js $
 $Revision: 23782 $
			...
			if (paramName=="TargetMovieClip")
			{
				...
				if (aClip!==null)
				{
					...
					if (aClip.getDivWrapper ().parentNode!==null)
					{
						...
						//pointer.getDivWrap().style.zIndex = CTATGlobalFunctions.gensym.z_index();

						pointer.ctatdebug ("Adding ...");

						pointer.getDivWrap().appendChild (aClip.getDivWrapper ());

						//var index= CTATGlobalFunctions.gensym.z_index(); //pointer.getCanvasZIndex (); // slightly wrong because the canvas doesn't exist anymore

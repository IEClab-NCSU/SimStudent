...
/**************************** PRIVATE INSTACE VARIABLES ******************************************************/
	 ...
	 * @type {float}
	 */
	var pGuess = Number(p_guess); if(Number.isNaN(pGuess)) { pGuess = CTATExampleTracerSkill.DEFAULT_P_GUESS };

	/**
	 * Float value for p_known.
	 * @type {float}
	 */
	var pKnown = Number(p_known); if(Number.isNaN(pKnown)) { pKnown = CTATExampleTracerSkill.DEFAULT_P_KNOWN };

	/**
	 * Float value for p_learn.
	 * @type {float}
	 */
	var pLearn = Number(p_learn); if(Number.isNaN(pLearn)) { pLearn = CTATExampleTracerSkill.DEFAULT_P_LEARN };

	/**
	 * Float value for p_slip.
	 * @type {float}
	 */
	var pSlip = Number(p_slip); if(Number.isNaN(pSlip)) { pSlip = CTATExampleTracerSkill.DEFAULT_P_SLIP };

	/**
	 * Bit vector value for right-wrong performance history on opportunities at this skill.
	 * @type {number}
	 */
	 ...
	 * Default value is SKILL_BAR_DELIMITER_v2_11.
	 * @type {string}
	 */
...
/***************************** PRIVILEDGED METHODS *****************************************************/
	...
	/**
	 * @return {string} skillName, the concatenation of category and name
	 */
	...
	/**
	 * @return {string} category, that part of the skill name after the space, if any
	 */
	...
	/**
	 * This method updates the history based on preset skill values.
	 * @param {string} status latest result to append, one of CORRECT, INCORRECT, HINT.
	 * @return {number} revised history
	 */
	...
	/**
	 * Format the Skill in XML for, e.g., responding to a ProblemSummaryRequest.
	 * @param {boolean} escape if true, substitute &amp; entities for &lt, &gt;.
	 * @return {string} XML element Skill
	 */
	this.toXML = function(escape)
	{
        ...
        attrs += ' pKnown="'      + Number(pKnown).toFixed(2)                 + '"';
        attrs += ' pLearn="'      + Number(pLearn).toFixed(2)                 + '"';
        attrs += ' pGuess="'      + Number(pGuess).toFixed(2)                 + '"';
        attrs += ' pSlip="'       + Number(pSlip).toFixed(2)                  + '"';
	 ...
	 * @return pKnown
	 */
        this.getPKnown = function() { return pKnown; }

	/**
	 * @return pLearn
	 */
	this.getPLearn = function() { return pLearn; }

	/**
	 * @return pGuess
	 */
	this.getPGuess = function() { return pGuess; }

	/**
	 * @return pSlip
	 */
	this.getPSlip = function() { return pSlip; }
	 ...
	 * @return opportunityCount
	 */
        this.getOpportunityCount = function() { return opportunityCount; }
	 ...
	 */
 	...
 	/**
 	 * @return {string} description if set; otherwise skillName
 	 */
...
/****************************** STATIC METHODS ****************************************************/
	...
	/**
	 * Updates the history using algorithm from Java implementation.
	 * This method can be called from anywhere returning the updated history value, but the result
	 * is not stored internally to this skill object.
	 * Maximum history length is MAX_INTEGER_SHIFT-1.
	 * @param {string} givenStatus latest result to append, one of CORRECT, INCORRECT, HINT.
	 * @param {number} givenHistory bit vector of existing history
	 * @return {number} revised bit vector, with latest result in lsb
	 */
	CTATExampleTracerSkill.updateHistoryStatic = function(givenStatus, givenHistory)
	{
		...
		return (h & msb ? h ^ msb : h);  // ensure result is positive
	};
...
/**
 * Constant to represent maximum bit shift for an integer..
 * @var {string} MAX_INTEGER_SHIFT
 */
...
/**
 * @var {string} SKILL_BAR_DELIMITER_v2_10
 */
 ...
 * @var {string} SKILL_BAR_DELIMITER_v2_11
 */
 ...
 * @var {float} DEFAULT_MASTERY_THRESHOLD
 */

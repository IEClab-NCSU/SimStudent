...
/***************************** PRIVATE METHODS *****************************************************/
	...
	function checkDoneStep(result, isHintTrace, doUpdate, newInterps)
	{
		...
		{
			result.setDoneStepFailed(false);  // always succeeds
			that.ctatdebug("checkDoneStep() exitOnIncorrectDone() with setDoneStepFailed(false), so return null ");
			return null;
		}
		if(!isHintTrace && graph.hideButRequireSteps())
		{
			...
			while(!((doneLink = iter.next()).done))
			{
				if(graph.pathToLinkIsSubset(doneLink.value, matchedSelections))
				{
					...
					{
						return finishEvaluate(result, doUpdate, CTATExampleTracerLink.CORRECT_ACTION, doneLink.value, that.getBestInterpretation(), that.getInterpretationsInternal());
					}
					else
					{
						return finishEvaluate(result, doUpdate, CTATExampleTracerLink.CORRECT_ACTION, doneLink.value, findBestInterpretation(newInterps), newInterps);
					}
				}
			}
		}
...
/***************************** PRIVILEDGED METHODS *****************************************************/
	...
	this.matchForHint = function(givenResult)
	{
		...
		var link = null;
		var selection = (givenResult.getStudentSAI() === null || typeof(givenResult.getStudentSAI()) === 'undefined' ? null : givenResult.getStudentSAI().getSelectionArray()); //of type array

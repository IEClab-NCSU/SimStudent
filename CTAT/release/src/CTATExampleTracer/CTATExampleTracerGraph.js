...
/**************************** PRIVATE INSTACE VARIABLES ******************************************************/
    ...
    /**
     * The origin node of the graph, the first state read from the brd file.
     * @type {CTATExampleTracerNode}
     */
    var startNode = null;

    /**
     * The state at which the student starts, different from startNode if this graph has a partially-worked example.
     * @type {CTATExampleTracerNode}
     */
	var studentStartsHereNode = null;

	/**
	 * Destination nodes of done steps
	 * @type {Set<CTATExampleTracerNode>}
	 */
	var doneStates = new Set();

	/**
	 * @type {Set<ExampleTracerLink>}
	 */
     ...
     * @type {string} one of CTATMsgType.SHOW_ALL_FEEDBACK, CTATMsgType.HIDE*
     */
    ...
    function initGraph(isOrdered, youStartYouFinish)
    {
        ...
        if (groupModel === null || typeof (groupModel) === 'undefined')
        {
            ...
            that.ctatdebug("CTATExampleTracerGraph --> created a group model from here");
    ...
    function isReenteringOK(traversedLinks, newLink, path)
    {
        ...
        if (traversedLinks.length !== 0)
        {
            ...
            that.ctatdebug("CTATExampleTracerGraph --> in isReentrableOK: exitedGroups null? " + (exitedGroups === null));
            that.ctatdebug("CTATExampleTracerGraph --> in isReentrableOK: exitedGroups undefined? " + (typeof(exitedGroups) === 'undefined'));
            that.ctatdebug("CTATExampleTracerGraph --> in isReentrableOK: exitedGroups last traversedLink " + traversedLinks[traversedLinks.length - 1].getUniqueID());
			that.ctatdebug("CTATExampleTracerGraph --> in isReentrableOK: exitedGroups length "+(exitedGroups ? exitedGroups.length : null));
			...
			for(k = exitedGroups.length-1; k >= 0; k--)
			{
				...
				{
					exitedGroups.splice(k, 1); //removes the current element from the array
				}
			}
			that.ctatdebug("CTATETGraph --> in isReentrableOK: exitedGroups after removing reenterables: "+exitedGroups.length);

			that.ctatdebug("CTATETGraph --> in isReentrableOK: traversedLinks: "+traversedLinks+", path: "+path);
            var retVal = true;
	...
	/**
	 * @param {string} policy one of CTATMsgType.SHOW_ALL_FEEDBACK, HIDE_ALL_FEEDBACK, DELAY_FEEDBACK;
	 *                        or "true" or "false" from old-style .brd files
     */
	this.setFeedbackPolicy = function(policy)
	{
		that.ctatdebug("Graph.setFeedbackPolicy("+policy+") old policy "+feedbackPolicy);
		if(policy === "false")     feedbackPolicy = CTATMsgType.SHOW_ALL_FEEDBACK;
		else if(policy === "true") feedbackPolicy = CTATMsgType.HIDE_ALL_FEEDBACK;
		else                       feedbackPolicy = policy;
	};

	/**
	 * @return {boolean} true if feedbackPolicy is CTATMsgType.HIDE_BUT_COMPLETE
	 */
	...
	/**
	 * @return {boolean} true if feedbackPolicy is CTATMsgType.HIDE_ALL_FEEDBACK
	 */
	...
	/**
	 * Whether the example tracer should skip tests for ordering or traversal constraints.
	 * @return {boolean} true if this.hideAllFeedback() or this.hideButRequireSteps() or
	 *                   feedbackPolicy == CTATMsgType.DELAY_FEEDBACK
	 */
	...
	/**
	 * Whether a tutor should exit on an incorrect done step.
	 * @return {boolean} true if feedbackPolicy is HIDE_ALL_FEEDBACK or HIDE_BUT_ENFORCE
	 */
	...
	/**
	 * Tell whether the path from the start state to the given link is a subset of the given
	 * set of links. If a link is not required, don't require it, but follow it.
	 * @param {CTATExampleTracerLink} link destination link
	 * @param {Set<CTATExampleTracerLink>} links superset of links to test
	 * @return true if links contains all required links in the path from this link to the start state
	 */
	this.pathToLinkIsSubset = function(link, links)
	{
		...
		if(link.getMinTraversals() > 0 && typeof(links[link.getUniqueID()]) == "undefined") // links[id] could be present but false
		{
			return false;
		}
		if(linkSource == that.getStartNode() /* || linkSource.isBeforeStartState() */ )
		{
			return true;
		}
		...
		{
			return that.pathToLinkIsSubset(prevLink.value, links);  // depth-first walk up the tree
		}
		return false;
	};
    ...
    this.observesOrderingConstraints = function(traversedLinks, newLink, path, result)
    {
		...
		{
			that.ctatdebug("CTATExampleTracerGraph --> in observesOrderingConstraints initializing out of order false");
		...
		if(newLink.isDone() && that.exitOnIncorrectDone())
		{
            that.ctatdebug("CTATExampleTracerGraph.observesOrderingConstraints --> returning true for doneStep && exitOnIncorrectDone");
			return true;
		}

        if ((path.has(newLink) === true) && (isOrderOK(traversedLinks, newLink, path) === true) && (isReenteringOK(traversedLinks, newLink, path) === true))
        {
            that.ctatdebug("CTATExampleTracerGraph.observesOrderingConstraints --> returning isOrderOK true && isisReenteringOK() true");
            return true;
        }

        if (result !== null && typeof (result) !== 'undefined')
        {
            that.ctatdebug("CTATExampleTracerGraph --> in observesOrderingConstraints setting out of order true");
            result.setOutOfOrder(true);
        }

        that.ctatdebug("CTATExampleTracerGraph --> returning false observesOrderingConstraints");
        return false;
    };
	...
	/**
	 * Slow way to find a node: linear lookup for matching name.
	 * Special case: if argument is "start" and no node so-named is found,
	 * return getStudentStartsHereNode() if set, or getStartNode());
	 * @param {string} nodeName node name, case insensitive
	 * @return {object<CTATExampleTracerNode>} node by this name; else null
	 */
	...
	/**
	 * @return {Set<CTATExampleTracerNode>} doneStates
	 */
	...
	/**
	 * Check whether the given link's destination node belongs in doneStates and insert it if so.
	 * @param {CTATExampleTracerLink} link to check
	 */
     ...
     * @param {CTATExampleTracerNode} fromNode source
     * @param {CTATExampleTracerNode} toNode destination
	 * @param {[CTATExampleTracerLink]} viaLinks if not null, require these links in the path chosen
     * @return path instance; null if no path exists
	 */
	this.getBestSubpath = function(fromNode, toNode, viaLinks)
	{
		if(!fromNode || !toNode)
			return null;
		var pathsFromNode = that.findPathsFromNode(fromNode);  // Set of CTATExampleTracerPath
		var pathsFromNodeToNode = new Set();                   // Set of CTATExampleTracerPath
    ...
    {
        that.ctatdebug("CTATExampleTracerGraph --> in setStartNode("+node+")");
        startNode = node;
    };

	/**
	 * @return {CTATExampleTracerNode} studentStartsHereNode
	 */
	this.getStudentStartsHereNode = function()
	{
        that.ctatdebug("CTATExampleTracerGraph --> getStudentStartsHereNode() returns "+studentStartsHereNode);
        return studentStartsHereNode;
	};

	/**
	 * @param {CTATExampleTracerNode} node new value for studentStartsHereNode
	 */
	this.setStudentStartsHereNode = function(node)
	{
        that.ctatdebug("CTATExampleTracerGraph --> in setStudentStartsHereNode("+node+")");

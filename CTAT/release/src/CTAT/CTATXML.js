/**-----------------------------------------------------------------------------
 ...
 $HeadURL: svn://pact-cvs.pact.cs.cmu.edu/usr5/local/svnroot/AuthoringTools/branches/CTAT_4_2_Release/HTML5/src/CTAT/CTATXML.js $
 $Revision: 24129 $
...
CTATXML = function()
{
	...
	this.parseXML=function parseXML (aMessage)
	{
		...
		try
		{
			//this.ctatdebug ("JQuery check: " + typeof($));

			if(typeof($)!=='undefined')
			{
				//this.ctatdebug ("Parse in a JQuery environment ...");
				xmlDoc = $.parseXML(aMessage);
			}
			else if(typeof(XMLParser)!=='undefined')
			{
				//this.ctatdebug ("Parse in a NodeJS environment ...");

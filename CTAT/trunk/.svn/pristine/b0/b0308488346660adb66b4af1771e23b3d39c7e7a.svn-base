...
silex.controller.ControllerBase.prototype.doAddElement = function(element) {
  ...
  if (!(element.className.includes('CTATGroupingComponent')
		&& !element.className.includes('container-element')))
 ...
 */
silex.controller.ControllerBase.prototype.save = function(saveAs, cbk) 
{
  ...
  let parent = JSON.parse(this.model.file.getMeta('parent-data'));
  if (!saveAs && (parent && (parent.id || parent.id === '')))
  {
	  var fileData = {
		type: 'text/html',
	  ...
	  cloudUtils.saveFile(
			filename, 
			false, 
			{htmlId: parent.id, path: parent.path}, 
			fileData, 
			cbk
	  );
  }
  ...
  {
	let pkgName = parent ? parent.path : "";
	if (pkgName.charAt(0) === '/')
		pkgName = pkgName.substring(1, pkgName.length);
	pkgName = pkgName.split('/');
	pkgName = pkgName[0];

	this.publishDialog.show('save', cbk, pkgName);
  }
};

 ...
 * @return {CSSRule|null}
 */
silex.model.Property.prototype.getRule = function(className) {
  if (!this.silexStyleSheet)
	  return;
...
silex.model.Property.prototype.updateSilexStyleTag = function(doc, opt_writeDom) {
  ...
  {
	let selector = this.silexStyleSheet.cssRules[idx].selectorText;
	let cssRule = this.getRule(selector);
	if (cssRule && cssRule.style)
		cssRule = cssRule.style.cssText;
	else
		cssRule = '';
	if (selector)
	{
		let styleStr = selector + ' {'+cssRule+'\n}\n';
		allStyles += styleStr;
	}
  }
 ...
 */
silex.model.Property.prototype.setStyle = function(selector, style) {
  if (!this.silexStyleSheet)
	  return;
  // convert style to string
  var styleStr = silex.utils.Style.styleToString(style || '');
  // we use the class name because elements have their ID as a css class too
  styleStr = selector + '{' + styleStr + '} ';
  // find the index of the rule for the given element
  var originalCssRuleIdx = -1;
  for (var idx in this.silexStyleSheet.cssRules) {
	if (this.silexStyleSheet.cssRules[idx].selectorText === selector) {
...
/**
*	Remove all style rules with selectors containing a given substring
*	@param substr the substring to match selectors against
*/
silex.model.Property.prototype.removeStylesBySubstring = function(substr)
{
	for (var idx = 0; idx < this.silexStyleSheet.cssRules.length; idx++) 
	{
		if (this.silexStyleSheet.cssRules[idx].selectorText.includes(substr)) 
		{
			this.silexStyleSheet.deleteRule(idx);
			idx--;
		}
	}
};
...
/**
*	Get all style rules with selectors containing a given substring
*	@param substr the substring to match selectors against
*/
silex.model.Property.prototype.getStylesBySubstring = function(substr)
{
	var styles = [];
	for (var idx = 0; idx < this.silexStyleSheet.cssRules.length; idx++) 
	{
		...
		{
			var thisStyle = {
				selector: this.silexStyleSheet.cssRules[idx].selectorText,
				style: silex.utils.Style.styleToObject(this.silexStyleSheet.cssRules[idx].style)
			};
 ...
 * @return {Object|null}
 */
silex.model.Property.prototype.getStyleObject = function (element, opt_computed, opt_selector) {
  var cssStyleDeclaration = null;
  if(opt_computed !== true) 
  {
	let selector = opt_selector || '.' + this.model.property.getSilexId(element);
    let cssRule = this.getRule(selector);
    if(cssRule) {
 ...
 * @param includeBorder whether to include border width in box size
 * @return {{top:?number, left:?number, width:?number, height:?number}} the bounding box containing all the elements
 */
silex.model.Property.prototype.getBoundingBox = function(elements, includeBorder) {
  ...
  goog.array.forEach(elements, function(element) {
	...
	if (includeBorder)
	{
		elementRight += parseInt(elementStyle["border-left-width"] || 0, 10);
		elementRight += parseInt(elementStyle["border-right-width"] || 0, 10);
		elementBottom += parseInt(elementStyle["border-top-width"] || 0, 10);
		elementBottom += parseInt(elementStyle["border-bottom-width"] || 0, 10);
	}

...
silex.view.Stage.prototype.backgroundElement = null;
...
silex.view.Stage.prototype.initEvents = function(contentWindow) {
  ...
  this.backgroundElement = this.bodyElement.querySelector('.background-initial');
  ...
  goog.events.listen(this.bodyElement, 'mouseup', function(event) {
	if (!this.controller.editMenuController.textEditFocused)
	{
		let x = event.clientX;
		let y = event.clientY;
  ...
  goog.events.listen(
    ...
    function(event) {
		...
		}.bind(this), silex.view.Stage.DBLCLICK_DELAY);
		event.preventDefault();
		return false;
	}, false, this);
};
silex.view.Stage.DBLCLICK_DELAY = 5;
...
silex.view.Stage.prototype.handleMouseUp = function(target, x, y, shiftKey) {
  ...
  if (this.isDragging || this.isResizing) {
	...
	goog.dom.classlist.remove(this.backgroundElement, 'silex-show-grid');
  }
  else if (this.lassoSelected)
  {
 ...
 */
silex.view.Stage.prototype.resetFocus = function(isRedraw) {
  var stage = this;
  this.invalidationManagerFocus.callWhenReady(() => {
	if (!stage.controller.editMenuController.textEditFocused || isRedraw)
	{
	  ...
	  if (this.isDown) 
	  {
		...
		else
		{
			...
			if (!this.isDragging && !this.isResizing) {
			  if (this.snapToGrid)
			  {
				  goog.dom.classlist.add(this.backgroundElement, 'silex-show-grid');
			  }
...
silex.view.Stage.prototype.multipleDragged = function(x, y, shiftKey, ctrlKey) {
  ...
  var didChangePos = {x: false, y: false}, 
	  didChangeSize = {x: false, y: false};
  // drag or resize
  if (this.isDragging || this.resizeDirection === null) {
	...
	doUpdateX = (offsetX === 0) ? false : doUpdateX;
	doUpdateY = (offsetY === 0) ? false : doUpdateY;
	if (doUpdateY || doUpdateX)
		didChangePos = this.followElementPosition(followers, offsetX, offsetY, {ctrlKey: ctrlKey});
  }
    ...
    let offsetY = y - this.lastPosY + (scrollY - this.lastScrollTop);
	if ((offsetX !== 0) || (offsetY !== 0))
		didChangeSize = this.followElementSize(followers, 
  ...
  if (didChangePos.x || didChangeSize.x)
  {
	this.lastPosX = x;
  }
  if (didChangePos.y || didChangeSize.y)
  {
	this.lastPosY = y;
  }
...
silex.view.Stage.prototype.followElementPosition =
  function(followers, offsetX, offsetY, e) 
  {
	...
	var updatedX=false, updatedY=false;
	var snapping = (this.snapToGrid && !(e && (e.ctrlKey || e.keyCode))) ? true : false;
    goog.array.forEach(followers, function(follower) {
	  ...
	  {
		 let pos = {}, newPos = {};
		 let roundX = (offsetX < 0) ? 'ceil' : 'floor';
		 let roundY = (offsetY < 0) ? 'ceil' : 'floor';
		 ...
		 if (snapping)
		 {
		    newPos.x = Math[roundX]((pos.x + offsetX)/20)*20;
			newPos.y = Math[roundY]((pos.y + offsetY)/20)*20;
		 }
		 else
		 {
			newPos.y = Math.round(pos.y + offsetY);
			newPos.x = Math.round(pos.x + offsetX);
		 }
		 if (newPos.y !== pos.y)
		 {
			this.controller.stageController.styleChanged('top', newPos.y + 'px', [follower], false);
			updatedY = true;
		 }
		 if (newPos.x !== pos.x)
		 {
			this.controller.stageController.styleChanged('left', newPos.x + 'px', [follower], false);
			updatedX = true;
		 }	
	  }
	}, this);
...
silex.view.Stage.prototype.followElementSize =
  function(followers, resizeDirection, offsetX, offsetY, e) {
  var didUpdateWidth=false, didUpdateHeight=false;
  var roundX = (offsetX < 0) ? 'ceil' : 'floor';
  var roundY = (offsetY < 0) ? 'ceil' : 'floor';
  var snapping = (this.snapToGrid && !(e && (e.ctrlKey || e.keyCode))) ? true : false;
  // apply offset to other selected element
  goog.array.forEach(followers, function(follower) {
    ...
    if (follower.tagName.toUpperCase() !== 'BODY'
      ...
      var pos = {top: this.model.element.getStyle(follower, 'top'), 
				 left: this.model.element.getStyle(follower, 'left')};
	  pos.left = parseInt(pos.left.substring(0, pos.left.length-2), 10);
      pos.top = parseInt(pos.top.substring(0, pos.top.length-2), 10);
	  offsetPosX = pos.left;
	  offsetPosY = pos.top;
	  ...
	  var right, bottom;
	  if ((offsetSizeX !== 0)
	  &&  (size.width + offsetSizeX >= silex.model.Element.MIN_WIDTH))
	  {
		newSize.width = size.width + offsetSizeX;
		if (snapping)
		{
			if (offsetPosX)
			{
				right = size.width + pos.left;
				offsetPosX = Math[roundX]((right-newSize.width)/20)*20;
				newSize.width = right - offsetPosX;
			}
			else
			{
				right = Math[roundX]((newSize.width+pos.left)/20)*20;
				newSize.width = right - pos.left;
			}
		}
	  }
	  else 
	  ...
	  if ((offsetSizeY !== 0)
	  &&  (size.height + offsetSizeY >= silex.model.Element.MIN_WIDTH))
	  {
		newSize.height = size.height + offsetSizeY;
		if (snapping)
		{
			if (offsetPosY)
			{
				bottom = size.height + pos.top;
				offsetPosY = Math[roundY]((bottom-newSize.height)/20)*20;
				newSize.height = bottom - offsetPosY;
			}
			else
			{
				bottom = Math[roundY]((newSize.height+pos.top)/20)*20;
				newSize.height = bottom - pos.top;
			}
		}
	  }
	  else 
	  ...
	  {
		this.controller.stageController.styleChanged('width', Math.round(newSize.width) + 'px', [follower], false);
		if (offsetPosX || offsetPosX === 0) 
			this.controller.stageController.styleChanged('left', Math.round(offsetPosX) + 'px', [follower], false);
		didUpdateWidth = true;
	  }
	  if (newSize.height !== size.height)
	  {
        this.controller.stageController.styleChanged('height', Math.round(newSize.height) + 'px', [follower], false);
		if (offsetPosY || offsetPosY === 0)
  ...
  return {x: didUpdateWidth, y: didUpdateHeight}
};
 ...
 */
silex.view.Stage.prototype.propertyChanged = function(doRedraw) {
  // update property tool box
  this.controller.stageController.updateView(doRedraw);
};
...
*/
silex.view.Stage.prototype.showFileSourceWindow = function(filetype, callbackOne, callbackTwo, optVal)
{
	if (!this.fileSourceWindow)
	{
		this.fileSourceWindow = new CTATImageSource('#img-source-dialog');
	}
	this.fileSourceWindow.show(callbackOne, callbackTwo, optVal);
...
silex.view.Stage.prototype.showConfirmDialog = function(msg, options)
{
	if (!this.confirmDialog)
	{
		this.confirmDialog = new CTATConfirmDialog('#ctat-confirm-dialog');
	}
	this.confirmDialog.show(msg, options);
};
...
silex.view.Stage.prototype.setStatus = function(status)
{
	...
	if (statusField)
	{
		if (status.length > 63)
		{
			...
			let last = words.pop()
			let rest = words.join(' ');
			let restLen = 63 - (last.length + 3);
			rest = rest.substring(0, restLen);
			status = rest+'... '+last;
		}
...
silex.view.Stage.prototype.setFilename = function(filename)
{
	...
	if (nameField)
	{
		...
		if (filename.indexOf('.ed.html') > -1)
			filename = filename.substring(0, filename.indexOf('.ed.html'));

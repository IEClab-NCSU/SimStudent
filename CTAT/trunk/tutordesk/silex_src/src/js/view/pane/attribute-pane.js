...
silex.view.pane.AttributePane.prototype.createAttrInput = function(attribute, selectedElement)
{
	...
	var ctatClass = silex.utils.CTAT.getCTATClassName(selectedElement);
	...
	var input = null;
	var initialVal = attribute['getterFunction'] ? 
		attribute['getterFunction'](selectedElement) : 
		selectedElement.getAttribute(attribute['attrName']);
	
	if (!initialVal)
	{
		initialVal = ctatClass ? 
					 CTATComponentData[ctatClass]['attrDefaults'][attribute['name']] || attribute['default'] :
					 attribute['default'];
	}
	switch(attribute['inputType'])
	{
		...
		case 'button':
			...
			if (initialVal && this.model.file.imgUrlMap[initialVal])
			{
				name  = this.model.file.imgUrlMap[initialVal]['name'];
				title = this.model.file.imgUrlMap[initialVal]['path'];
			}
			else if (initialVal && (initialVal.includes('http://') || initialVal.includes('https://')))
			{
				title = initialVal;
				name  = initialVal.split('/').pop().split('?')[0];
			}
...
silex.view.pane.AttributePane.prototype.buildAttrArray = function(attributes)
{
	...
	for (let i = 0; i < baseGroups.length; i++)
	{
		...
		if (baseGroups[i] === 'ID')
		{
			attrArray = CTATComponentData['attributeGroups']['ID'].concat(attrArray);
		}
		else if (baseGroups[i] === 'Base')
		{
			attrArray = attrArray.concat(CTATComponentData['attributeGroups']['Base']);
		}
		else
		{
			attributes = CTATComponentData['attributeGroups'][baseGroups[i]].concat(attributes);
		}
	}
...
silex.view.pane.AttributePane.prototype.getAttributeList = function(className)
{
	...
	for (var i = 0; i < numClasses; i++)
	{
		...
		if (CTATComponentData.hasOwnProperty(thisClass))
		{
			if (thisClass.includes("CTAT"))
			{
				allAttr = CTATComponentData[thisClass]['attributes'].concat(allAttr);
			}
			else
			{
				allAttr = allAttr.concat(CTATComponentData[thisClass]['attributes']);
			}
		}
	}
...
silex.view.pane.AttributePane.prototype.updateAttr = function(attribute, optEl)
{
	...
	this.setAttrValue(attribute, {name: val})
}

silex.view.pane.AttributePane.prototype.setAttrValue = function(attribute, data)
{
	let attrDiv = this.element.querySelector('div[data-silex-attr-name="'+attribute+'"]');
	let valElement = $(attrDiv).find('.silex-asset-label')[0];
	if (valElement)
	{
		valElement.textContent = data.name;
		valElement.title = data.path;
	}
	else
	{
		let val = data.name;
		valElement = $(attrDiv).find('input')[0];
		if (valElement)
		{

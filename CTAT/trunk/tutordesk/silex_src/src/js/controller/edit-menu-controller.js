...
silex.controller.EditMenuController.prototype.editTextElement = function(element)
{
	//get current contents
	var contentNode = $(element).find('.silex-element-content')[0];
	var text = contentNode.innerHTML;
	text = text.replace(/<br>/g, '\n') || '';
	//create textarea
	var input = this.model.file.getContentDocument().createElement('textarea');
	input.classList.add('text-edit-input');
	input.id = 'editor-for'+element.getAttribute('data-silex-id');
	//append to element
	element.removeChild(contentNode);
	...
	input.addEventListener('blur', function()
	{
		...
		contentNode.innerHTML=input.value.replace(/\n/g, '<br>');
		this.textEditFocused = false;
	}.bind(this));

	this.textEditFocused = true;
	input.focus();
}
...
silex.controller.EditMenuController.prototype.toggleLasso = function(event)
{
	...
	let lassoStyle = "*{cursor: crosshair !important;}"
	if (wasSelected)
	{
		button.classList.add('off');
		this.model.head.removeTempStyle(lassoStyle);
	}
	else
	{
		button.classList.remove('off');
		this.model.head.addTempStyle(lassoStyle);
	}
}

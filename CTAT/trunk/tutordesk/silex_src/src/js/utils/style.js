...
silex.utils.Style.styleToObject2 = function(styleObj) {
  var res = {};
  var regex = /([^:\s]*): ?([^;]*);?/g;
  var match;
  do {
	  match = regex.exec(styleObj.cssText);
	  if (match)
		res[match[1]] = match[2];
  }while(match);
  return res;
...
silex.utils.Style.expandShorthand = function(shorthand)
{
	if (!shorthand)
		return null;
	
	var arr = shorthand.split(' ');
	var val = '';
	switch (arr.length)
	{
		case 1:
			val = arr[0]+' '+arr[0]+' '+arr[0]+' '+arr[0];
		break;
		case 2:
			val = arr[0]+' '+arr[1]+' '+arr[0]+' '+arr[1];
		break;
		case 3:
			val = arr[0]+' '+arr[1]+' '+arr[2]+' '+arr[1];
		break;
		case 4:
			val = arr.join(' ');
	}
...
/**
*	Get the total width of an element including border
*	@param el the element
*/
silex.utils.Style.getTotalWidth = function(el)
{
	return $(el).outerWidth();
}

/**
*	Get the total height of an element including border
*	@param el the element
*/
silex.utils.Style.getTotalHeight = function(el)
{
	return $(el).outerHeight();
}

silex.utils.Style.canonicalToHexMap = {
	'black': '#000000',
	'blue': '#0011ff',
	'red': '#ff0000',
	'green': '#008000',

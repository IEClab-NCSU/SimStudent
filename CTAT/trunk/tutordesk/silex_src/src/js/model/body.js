...
silex.model.Body.prototype.background = null;
...
*/
silex.model.Body.prototype.getBackground = function()
{
  if (!this.background)
	this.background = goog.dom.getElementByClass('background-initial', this.model.file.getContentDocument());

  return this.background;
};

silex.model.Body.prototype.setBackground = function(bg)
{
	this.background = bg;
};
 ...
 */
silex.model.Body.prototype.setSelection = function(selectedElements, redrawStage) {
  // reset selection
  var elements = goog.dom.getElementsByClass(silex.model.Element.SELECTED_CLASS_NAME, this.getBodyElement());
  goog.array.forEach(elements, function(element) {
...
silex.model.Body.prototype.setEditable = function(rootElement, isEditable) {
  ...
  //add temporary inner containers to all scrolling containers
  if (isEditable)
  {
	elements = goog.dom.getElementsByClass('container-element', rootElement);
	var innerContainer;
	goog.array.forEach(elements, function(element)
		{
			if (element.className.includes('silex-container-scrollx'))
			{
				pointer.model.element.setScroll(element, 'x', true);
				element.classList.remove('silex-container-scrollx');
			}
			if (element.className.includes('silex-container-scrolly'))
			{
				pointer.model.element.setScroll(element, 'y', true);
				element.classList.remove('silex-container-scrolly');
			}
		});
  }
};
...
silex.model.Body.prototype.removeEditableClasses = function(rootElement) {
  if (rootElement !== null) {
	...
	//remove temp inner containers in scrolling containers
	elements = goog.dom.getElementsByClass('scrollcontainer-inner', rootElement);
	goog.array.forEach(elements, function(element)
		{
			...
			if (element.className.includes('silex-container-scrollx'))
			{
				element.parentNode.classList.add('silex-container-scrollx');
			}
			if (element.className.includes('silex-container-scrolly'))
			{
				element.parentNode.classList.add('silex-container-scrolly');
			}

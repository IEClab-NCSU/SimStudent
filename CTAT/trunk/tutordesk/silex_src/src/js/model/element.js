 ...
 * @type {number}
 */
silex.model.Element.MIN_HEIGHT = 5;
 ...
 * @type {number}
 */
silex.model.Element.MIN_WIDTH = 5;
...
silex.model.Element.prototype.getStyle = function(element, styleName, opt_computed) 
{
  ...
  {
	  //if we have selector but no stored style, try jquery instead
	  return this.model.file.getContentWindow().$(selector).css(cssName);
  }
  return null;
};
...
silex.model.Element.prototype.setStyle = function(element, styleName, opt_styleValue) 
{
		 ...
		 || (styleName.includes('border') && styleName.toLowerCase().includes('width')))
	{
		...
		//re-render if needed
		if (silex.Config.needsRendered[silex.utils.CTAT.getCTATClassName(element)])
		{
...
silex.model.Element.prototype.getBgImage = function(element)
{
	var url = this.getStyle(element, 'backgroundImage');
	url && (url = silex.utils.Url.removeUrlKeyword(url));
	return url || '';
}
...
silex.model.Element.prototype.createCtatElement = function(type, defaultStyle)
{
	...
	switch(type)
	{
		...
		break;
		case 'numericstepper':
			ctatClass = 'CTATNumericStepper';
			defaultStyle.width = '60px';
			defaultStyle.height = '23px';
		break;
		case 'video':
			ctatClass = 'CTATVideo';
			defaultStyle.width = '250px';
			defaultStyle.height = '200px';
			defaultStyle.paddingBottom = '2px';
			defaultStyle.paddingLeft = '2px';
			defaultStyle.paddingRight = '2px';
		break;
		case 'groupingcontainer':
			ctatClass = 'CTATGroupingComponent';
			element.setAttribute('data-silex-type', 'container');
			element.classList.add('container-element');
		break;
	}

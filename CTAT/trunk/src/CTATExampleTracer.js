/**-----------------------------------------------------------------------------
 $Author: mdb91 $
 $Date: 2017-06-06 16:23:59 -0500 (週二, 06 六月 2017) $
...
CTATExampleTracer = function()
{
	 ...
	 */
	function handleMessageFromInterface(aMessage, restoring) //2
    {
        ...
        case CTATMsgType.PROBLEM_SUMMARY_REQUEST:
			setTimeout(function(problemName)  // delay response 2 sec to allow done step's logging to complete
			{
	...
	*/
	...
	this.processNools = function(noolsFile)
	{
		...
		{
			var startStateEndMsgStr = "<message><verb>SendNoteProperty</verb><properties><MessageType>StartStateEnd</MessageType></properties></message>",
				sgMsg = setConfig(problemConfig),
				endStartStateBundle = [];
			
			maybeHideHintButton(startStateMsgList);  // appends msg if feedback suppressed
		...
		ruleTracer = new CTATRuleTracer('nools');
		ruleTracer.initEngine(noolsFile, (startStateMsgs, stateGraphMsg)=>{
	...
	function setConfig(configObj) {
		var caseInsensitive = "false",
			isUnordered = "false",
			msgBuilder = new CTATTutorMessageBuilder();
		...
		that.setHighlightRightSelection(highlightRightSelection != "false");  // default is true
		...
		return(msgBuilder.createStateGraphMessage(caseInsensitive,
												isUnordered,
												lockWidget,
												that.isFeedbackSuppressed(),
	 ...
	 * @param xml incoming message as XML element
	 */
	function handleInterfaceAction(aMessage, message, parser, xml) //3
	{
		...
		if (selectionArray[0] && (selectionArray[0].toLowerCase() == "hint" || selectionArray[0].toLowerCase() == "help"))
		{
			...
			var edge = doHint(selectionArray, actionArray, "student", true, transactionID);
	/*		
			--- Moved to hintDone ---
			...
			var assocMsg = new CTATTutorMessageBuilder().createAssociatedRulesMessageForHint(hints,
																							result.getTutorSAI(),
																							actorForLog(CTATMsgType.DEFAULT_STUDENT_ACTOR),
																							that.getSkillBarVector(true),
																							stepID,
																							transactionID);
	...
	*/	
		...
		else  //Process a interface action that is not a hint
		{
            ...
            doNewTrace(tutorType,[null, message.getSAI(), "student", true, transactionID, aMessage]);
		}
		return true;
	}
...
//Tutor performed actions
	...
	*/
	function doHint (givenSelection, givenAction, givenActor, allowHintBias, transID)
	{
		...
		if(allowHintBias && givenAction && (givenAction.length > 1) && (givenAction[1].toString() === CTATMsgType.PREVIOUS_FOCUS.toString()) 
	    && givenSelection && (givenSelection.length > 1) && givenSelection[1] && (givenSelection[1].toString() !== "null")) //ouch
		{
			previousFocus = (givenSelection[1] !== givenSelection[0]) ? [givenSelection[1]] : null;

			if(givenAction.length > 2)
			{
				that.ctatdebug("CTATExampleTracer --> in doHint inner if condition");

				previousAction = [];
				for(var i = 2; i < givenAction.length; i++)
				{
					that.ctatdebug("CTATExampleTracer --> in doHint for loop " + i);
					previousAction.push(givenAction[i]);
				}
			}
		}
		that.ctatdebug("CTATExampleTracer --> in doHint out of if condition");
		...
		if (tutorType === 'example')
		{
			...
			hintDone(result, transID, ret);
		}
		else
		{
			ruleTracer.doHint(result, transID, hintDone);
		}
	}
	
	/**
	*	Process the result of a hint request
	*	@param {CTATExampleTracerEvent} result the result
	*	@param transactionID
	*	@param optEdge an example tracer link (only relevant to graph-based tutors ??)
	*/
	function hintDone(result, transactionID, optEdge)
	{
		var hints = result.getReportableHints();
		var stepID = (optEdge ? ""+optEdge.getUniqueID() : "");
		result.setStepID(stepID);
        
        that.updateSkills(CTATExampleTracerSkill.HINT, (optEdge ? optEdge.getSkillNames() : []), stepID);
	...
	*/
	function doNewTrace(traceType, args)
	{
		...
		var link = args.shift(),
			sai = args.shift(),
			actor = args.shift(),
			doTutorPerformedSteps = args.shift(),
			transactionID = args.shift(),
			origMsgTxt = args.shift();
		...
		switch(traceType) {
			...
			case 'rule':
				...
				doNewRuleTrace(result, origMsgTxt, finish);  // should return a value?
			break;
		}
	}
     ...
     */
    function doNewExampleTrace (link, /*sai,actor,doTutorPerformedSteps,transactionID,*/ result, origMsgTxt, transactionID)
    {
...
*/
	...
	function doNewRuleTrace(result, origMsgTxt, cbk) //5
	{
    ...
    function finishNewExampleTrace(result, sai, actor, doTutorPerformedSteps)
    {
        ...
        var msgBuilder = new CTATTutorMessageBuilder();
        var stepID = "";
		var link = result.getReportableLink() || null; //Equivalent to java edgeData = edges[0].getEdgeData();
		var tpaResult = null;
        ...
        else
        {
			...
			{
				tutorSAI = result.getTutorSAI();  // revise with silent hint result
				...
				var rulesMsgForAction = msgBuilder.createAssociatedRulesMessageForAction(result.getResult(),
																						(tutorSAI && tutorSAI.getSelection() ? tutorSAI : studentSAI),
																						actorForLog(actor),
																						studentSAI,
																						that.getSkillBarVector(true, actor),
																						stepID, 
																						transactionID, 
																						textResp.tutorAdvice);

/**-----------------------------------------------------------------------------
 $Author: sewall $
 $Date: 2017-01-10 10:56:54 -0600 (週二, 10 一月 2017) $
 $HeadURL: svn://pact-cvs.pact.cs.cmu.edu/usr5/local/svnroot/AuthoringTools/branches/CTAT_4_2_Release/HTML5/src/CTATCommunication/CTATMessageHandler.js $
 $Revision: 24496 $
					...
					if (messageParser.getElementName (propNode)=="MessageType")
					{
						...
						if (nodeValue.indexOf ("InterfaceAction")!=-1)
						{
							...
							if (testSAI.getAction ().indexOf (":")!=-1)
							{
								...
								setTimeout (commShell.propagateShellEvent,targetTime,"InterfaceAction",aMessage,"tutor");
							}
							else
							{
								...
								else
								{
									...
									commShell.propagateShellEvent ("InterfaceAction",aMessage,"tutor");
						...
						if (nodeValue=="TutoringServiceError")
						{
							...
							commShell.propagateShellEvent ("TutoringServiceError",aMessage);
						}

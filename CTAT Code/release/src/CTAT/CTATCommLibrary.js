/**-----------------------------------------------------------------------------
 $Author: sewall $
 $Date: 2016-12-09 17:12:05 -0600 (週五, 09 十二月 2016) $
 $HeadURL: svn://pact-cvs.pact.cs.cmu.edu/usr5/local/svnroot/AuthoringTools/branches/CTAT_4_2_Release/HTML5/src/CTAT/CTATCommLibrary.js $
 $Revision: 24429 $
...
CTATCommLibrary = function(aHandler, aUseScrim)
{
	...
	*
	*/
	this.createConnection=function createConnection (aVars,aURL)
	{
	...
	*/
	...
	this.retrieveXMLFile = function retrieveXMLFile(xmlFile, parser, handler)
	{
		...
		if (location.href)
		{
			...
			{
				var errFile = new CTATTutorMessageBuilder().createErrorMessage("",
						"You have loaded your tutor directly from the file system, please do not load a tutor using file://. Please use the CTAT Authoring Tools to launch the html file.");
		...
		if (xmlFile.toLowerCase().indexOf ("file://")!=-1)
		{
			var errXFile = new CTATTutorMessageBuilder().createErrorMessage("",
					"Unable to load: "+xmlFile+" - You are trying to use the file:// protocol, which is not allowed in this browser.");
	...
	this.retrieveFile = function retrieveFile(txtFile, handler)
	{
		...
		{
			var errTFile = new CTATTutorMessageBuilder().createErrorMessage("",
					"Unable to load: "+txtFile+" - You are trying to use the file:// protocol, which is not allowed in this browser.");

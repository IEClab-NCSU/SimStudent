 ...
 */
silex.view.pane.BorderPane.prototype.redraw = function(selectedElements, pageNames, currentPageName, disable)
{
  ...
  if (!disable.includes('border'))
  {
	  ...
	  var borderWidth;	  
	  borderWidth = this.getCommonProperty(
		  selectedElements,
	  ...
	  if (borderWidth) 
	  {
		...
		// border color
		var borderColor;
		borderColor = this.getCommonProperty(
			selectedElements,
			goog.bind(function(element) {
				var w;
	  ...
	  // border style
	  var borderStyle;
		borderStyle = this.getCommonProperty(
		selectedElements,
		goog.bind(function(element) {
			var w;
...
silex.view.pane.BorderPane.prototype.onBorderWidthChanged = function() {
  if (this.borderWidthInput.value &&
      this.borderWidthInput.value !== '') 
  {
    if (this.borderWidthInput.value !== '0')
	{	
		// border color
		this.colorPicker.setEnabled(true);
	}
...
silex.view.pane.BorderPane.prototype.onBorderStyleChanged = function() {
  ...
  if (this.borderStyleComboBox.getSelectedItem())
  {
	this.styleChanged(
		'borderStyle',
...
silex.view.pane.BorderPane.prototype.onBorderCornerChanged = function() {
  ...
  if (goog.isDef(this.borderRadiusInput.value) &&
      this.borderRadiusInput.value !== '') {
    // corner placement
    var borderWidthStr = this.borderRadiusInput.value+'px';
...
silex.view.pane.BorderPane.prototype.toggleColorPaletteVisibility = function() {
  let visible = this.getColorPaletteVisibility(this.hsvPalette);
  if (!visible)
  {
	  this.setColorPaletteVisibility(this.controller.propertyToolController.view.propertyTool.bgPane.fgHsvPalette, false);
	  this.setColorPaletteVisibility(this.controller.propertyToolController.view.propertyTool.bgPane.bgHsvPalette, false);
  }
  this.setColorPaletteVisibility(this.hsvPalette, !visible);
};

/**
*	Disable the border pane inputs
*/
silex.view.pane.BorderPane.prototype.disable = function()
{
	console.log('borderPane.disable()');
	//color
	this.colorPicker.setEnabled(false);
	this.setColorPaletteVisibility(this.hsvPalette, false);
	//width
	this.borderWidthInput.setAttribute('disabled', 'yes');
	//radius
	this.borderRadiusInput.setAttribute('disabled', 'uh huh');
	//position checkboxes
	for (let i = 0; i < this.borderPlacementCheckBoxes.length; i++)
	{
		this.borderPlacementCheckBoxes[i].setEnabled(false);
	}
	//style
	this.borderStyleComboBox.setEnabled(false);
};

/**
*	Enable the border pane inputs
*/
silex.view.pane.BorderPane.prototype.enable = function()
{
	//color
	this.colorPicker.setEnabled(true);
	this.setColorPaletteVisibility(this.hsvPalette, this.getColorPaletteVisibility(this.hsvPalette));
	//width
	this.borderWidthInput.removeAttribute('disabled');
	//radius
	this.borderRadiusInput.removeAttribute('disabled');
	//position checkboxes
	for (let i = 0; i < this.borderPlacementCheckBoxes.length; i++)
	{
		this.borderPlacementCheckBoxes[i].setEnabled(true);
	}
	//style
	this.borderStyleComboBox.setEnabled(true);
}

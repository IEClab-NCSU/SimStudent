/**-----------------------------------------------------------------------------
 $Author: mringenb $
 $Date: 2016-06-16 11:19:27 -0400 (Thu, 16 Jun 2016) $
 $Header$
 $Name$
 $Locker$
 $Log$
...
CTATTutor.initializeHTMLComponents = function()
{
	...
	for (var CTATComponentType in CTAT.ComponentRegistry)
	{
		...
		$('.'+CTATComponentType).each(function()
		{
			...
			if (!$(this).data('CTATComponent')) { // assume initialized if already has CTATComponent (needed for CTATTable)
			...
			else
			{
				...
				ctat_component.setName(this.id);
			}
...
CTATTutor.createInterface=function createInterface ()
{
	...
	if (ignoreInterfaceDescriptions==true)
	{
		...
		{
			ctatdebug ("CRITICAL: CTATScrim.scrim is null!");
		}
 ...
 *
 */
CTATTutor.runTutor = function(aVars)
{
	ctatdebug ("runTutor ()");
...
CTATTutor.initTutor = function(aFlashVars,aDiv,aCanvas,usingFlash) // Main entry point
{
	...
	{
		//console.log ("Tutor already initialized, probably in author mode");
		return;
	}

	CTATConfiguration.generateDefaultConfigurationObject();
	CTATConfiguration.setParams(aFlashVars);

	if(usingFlash == null)
	{
		var paramNames = ['student_interface', 'swf_name'];  // add more UI names here
		usingFlash = CTATConfiguration.getSingleParameterValues(paramNames).some(function(val) {return val.search(/[.]swf$/i) >= 0;});
	}
	...
	var tempFlashVars=CTATConfiguration.getRawFlashVars();
	...
	if(tempFlashVars["swf_name"] && !window["sendToTutor"])
	{
		...
		window.registerInterface = function(msgHandler) {
			//console.log("window.registerInterface(" + msgHandler + ") CTAT.ToolTutor.tutor " + CTAT.ToolTutor.tutor);
	...
	if(String(tempFlashVars["tutoring_service_communication"]).toLowerCase() == "javascript")
	{
	...
	if (tempFlashVars ["tutoring_service_communication"]=="applet")
	{
	...
	if(!usingFlash)
	{
		CTATTutor.runTutor (tempFlashVars);
	}

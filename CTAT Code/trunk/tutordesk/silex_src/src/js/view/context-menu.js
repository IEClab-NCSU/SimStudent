 ...
 */
silex.view.ContextMenu = function(element, model, controller, view) {
  // store references
  /**
   * @type {Element}
   */
...
silex.view.ContextMenu.prototype.buildUi = function() {
    this.element.querySelector('.save').addEventListener('click', () => {this.controller.fileMenuController.save(false)});
	...
	this.element.querySelector('.js').addEventListener('click', () => {this.controller.insertMenuController.pickFile('script')});
	this.element.querySelector('.css').addEventListener('click', () => {this.controller.insertMenuController.pickFile('stylesheet')});
	this.element.querySelector('.grid').addEventListener('click', (e) => {this.controller.editMenuController.toggleSnapToGrid(e)});
	this.element.querySelector('.lasso').addEventListener('click', (e) => {this.controller.editMenuController.toggleLasso(e)});
	this.element.querySelector('.demonstrate').addEventListener('click', () => {this.controller.fileMenuController.demonstrate()});
	this.element.addEventListener(
		'click', 
		this.controller.componentPaletteController.unsetSelection.bind(this.controller.componentPaletteController),
		true
	);
};
...
silex.view.ContextMenu.prototype.redraw = function(opt_selectedElements, opt_pageNames, opt_currentPageName) {
  this.invalidationManager.callWhenReady(() => {
	...
	{		
      if (opt_selectedElements[0].tagName.toLowerCase() === 'body') 
	  {
	  ...
	  else if (opt_selectedElements[0].getAttribute('data-silex-id') === 'background-initial')
	  {
		this.element.querySelector('.delete').classList.add('off');
		this.element.querySelector('.copy').classList.add('off');
	  }

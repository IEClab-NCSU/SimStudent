...
/*was CTATComponentAttributes*/
var CTATComponentData = {
	'defaultStyles': {
		'margin': 'margin',
			'marginTop': 'marginTop',
	...
	'attributeGroups': {
		...
		'Base': [
			...
			},
			{
				...
				'default': 'true',
				'setterFunction': null/* function(el, value, input)
				{
				...
				'setterFunction': function(element, value)
					{
						...
						let currVal = element.getAttribute('data-ctat-src') || '';
						if (currVal && silexApp.model.file.imgUrlMap[currVal])
							currVal = '';
						silexApp.view.stage.showFileSourceWindow('audio', 
														function(url)
														{
															silexApp.controller.propertyToolController.setUrl(element, url, 'data-ctat-src');
														},
														function(fileData) {
															this.setBlobUrl(element, fileData, 'data-ctat-src');
	...
	'CTATImageButton': {
		'attributes': [
			...
			},
			{
				...
				'setterFunction': function(element, value)
				{
					let currVal = element.getAttribute('data-ctat-image-default') || '';
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('image',
															function(url)
															{
																silexApp.controller.propertyToolController.setUrl(element,
																													url,
																													'data-ctat-image-default');
															},
															function(fileData)
															{
																silexApp.controller.propertyToolController.setBlobUrl(element,
																														fileData,
																														'data-ctat-image-default');
															},
															currVal);
			...
			},
			{
				...
				'setterFunction': function(element, value)
				{
					let currVal = element.getAttribute('data-ctat-image-hover') || '';
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('image',
															function(url)
			...
			},
			{
				...
				'setterFunction': function(element, value)
				{
					let currVal = element.getAttribute('data-ctat-image-clicked') || '';
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('image',
															function(url)
			...
			},
			{
				...
				'setterFunction': function(element, value)
				{
					let currVal = element.getAttribute('data-ctat-image-disabled') || '';
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('image',
															function(url)
	...
	'CTATComboBox': {
		'attributes': [
			['ID', 'Class', 'Base', 'Tabbable'],
			{
				...
				'default': '',
				'setterFunction': null,
				'getterFunction': function(element)
				{
					let options = element.getAttribute('data-ctat-labels');
					if (options)
					{
						return options.split(',').length;
					}
					return null;
				}
			},
	...
	'CTATNumericStepper': {
		'attributes': [
			...
			},
			{
				...
				'attrName': 'step',
				'inputType': 'numeric',
				'description': 'The amount the value is changed by when arrows are clicked',
				'default': '1'
			}
		],
		'styles': {
	...
	'CTATVideo': {
		'attributes': [
			...
			},
			{
				...
				'attrName': 'src',
				'inputType': 'button',
				'description': 'The source file or URL of the video',
				'default': '',
				'setterFunction': function(element, value)
				{
					let currVal = element.getAttribute('src') || '';
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('video',
															function(url)
															{
																silexApp.controller.propertyToolController.setUrl(element, url, 'src');
															},
															function(fileData)
															{
																silexApp.controller.propertyToolController.setBlobUrl(element, fileData, 'src');
															},
															currVal);
				}
			},
			{
				'name': 'Autoplay',
				'attrName': 'data-ctat-autoplay',
				'inputType': 'checkbox',
				'description': 'Whether the video plays automatically on load',
				'default': 'true',
				'setterFunction': function(element, value)
					{
						silexApp.model.file.getContentWindow().CTATTutor.callComponentFunction(element,
						function(comp, val)
							{
								comp.setAutoplay(val);
							}, value);
					}
			},
			{
				'name': 'Show Controls',
				'attrName': 'data-ctat-controls',
				'inputType': 'checkbox',
				'description': 'Whether a control panel will be displayed on the component',
				'default': 'true',
				'setterFunction': function(element, value)
					{
						silexApp.model.file.getContentWindow().CTATTutor.callComponentFunction(element,
	...
	'container-element': {
		'attributes': [
			...
			{
				'name': 'Is CTAT Group',
				'attrName': 'data-silex-is-group',
				'inputType': 'checkbox',
				'description': 'Whether the container is a CTATGroupingComponent',
				'default': 'false',
				'setterFunction': function(element, value, input)
				{
					if (input.checked)
					{
						element.classList.add('CTATGroupingComponent');
						silexApp.model.file.getContentWindow().CTATTutor.initializeHTMLComponents()
					}
					else
					{
						element.classList.remove('CTATGroupingComponent');
						element.classList.remove('CTATComponent');
					}
					silexApp.view.propertyTool.setComponentType([element]);
					silexApp.view.propertyTool.attributePane.setAttrValue('id', {name: element.id || ''});
				}
			} */
		],
		'attrDefaults': {}
	...
	'image-element': {
		'attributes': [
			...
			{
				'name': 'Img Src',
				'attrName': 'data-silex-img-src',
				'inputType': 'button',
				'description': 'The source file or URL of the image',
				'default': '',
				'setterFunction': function(element, value)
				{
					let currVal = this['getterFunction'](element);
					if (currVal && silexApp.model.file.imgUrlMap[currVal])
						currVal = '';
					silexApp.view.stage.showFileSourceWindow('image',
															function(url)
															{
																silexApp.controller.propertyToolController.setImgUrl(url, null, element);
															},
															function(fileData)
															{
																silexApp.controller.propertyToolController.setBlobImgUrl(fileData, null, element);
															},
															currVal);
				},
				'getterFunction': function(element)
...
CTATComponentStyleMappings = {
	...
	'CTATNumericStepper': {
		...
		'styleAttributes': {
			...
			'fontWeight': ['font-weight', 'input'],
			'fontStyle': ['font-style', 'input'],
			'textAlign': ['text-align', 'input'],
			'color': ['color', 'input'],

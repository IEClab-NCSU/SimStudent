...
{  
    ...
    //cache.remove("first");
    //cache.put ("first", "v_-1");
  
    //cache.deleteProperty("first");
    cache.setProperty ("first", "v_-1");  
  
    //cache.remove("last");
    //cache.put ("last", "v_-1");
  
    //cache.deleteProperty("last");
    cache.setProperty ("last", "v_-1");  
}
...
{  
	...
	//var firstKeyRaw = cache.get("first");
	//var lastKeyRaw = cache.get("last");
  
	var firstKeyRaw = cache.getProperty("first");
	var lastKeyRaw = cache.getProperty("last");  
  
    var firstKey=Number (firstKeyRaw.split ("_")[1]);
    var lastKey=Number (lastKeyRaw.split ("_")[1]);
      
	for (var key = firstKey; key <= lastKey; key++) 
	{
        var target='k_'+key;
      
        ctatdebug ("Removing: " + target);
      
        //cache.remove(target);
        cache.deleteProperty(target);
	}
...
{ 
    ...
    //ctatdebug ("First: " + cache.get("first") + ", Last: " + cache.get("last"));
    ctatdebug ("First: " + cache.getProperty("first") + ", Last: " + cache.getProperty("last"));
    ...
    if (firstKeyRaw==null)
    {
      ctatdebug ("Internal error: firstKeyRaw==null");
      return;
    }
  
    if (lastKeyRaw==null)
    {
      ctatdebug ("Internal error: lastKeyRaw==null");
      return;
    }  
	...
	for (var key = firstKey; key <= lastKey; key++) 
	{
        ...
        //var entity=cache.get(target);
        var entity=cache.getProperty(target);
      
        ctatdebug("key = " + target + " value = " + entity);
	}
}
...
{    
    ...
    ctatdebug ("First: " + firstKey + ", Last: " + firstKey);
    ...
    if (lastKey==-1)
    {
      ...
      //cache.remove("first");
      //cache.put ("first", "v_"+firstKey.toString ());
      cache.setProperty ("first", "v_"+firstKey.toString ());
      
      //cache.remove("last");
      //cache.put ("last", "v_"+lastKey.toString ());
      cache.setProperty ("last", "v_"+lastKey.toString ());
      
      var newIndexKey=('k_'+lastKey);
      
      ctatdebug ("Assigning ["+newIndexKey+"]: " + entry);
      
      //cache.put (newIndexKey, entry.toString ());	  
      cache.setProperty (newIndexKey, entry.toString ());	  
            
      ctatdebug ("Result: " + cache.getProperty(newIndexKey) + " : " + typeof newIndexKey);
    }  
    else
    {
	  lastKey++;
      ...
      ctatdebug ("Adding ["+newIndexKey+"]: " + entry);
      
      //cache.put (newIndexKey, entry.toString ());          
      cache.setProperty (newIndexKey, entry.toString ());          
      
      //ctatdebug ("Result: " + cache.get(newIndexKey) + " : " + typeof newIndexKey);
      ctatdebug ("Result: " + cache.getProperty(newIndexKey) + " : " + typeof newIndexKey);
    }
...
function dequeueOnEdit() 
{
	...
	for (var key = firstKey; key <= lastKey; key++) 
	{
       var entity=cache.getProperty('k_'+key);
          
       if (entity!=null)
       {      
          ctatdebug("key = " + key + " value = " + entity);
    ...
    ctatdebug ("Returning composed string: " + formatted );
...
function lengthOnEditQueue() 
{
    ...
    */
    ...
    var firstKey = Number(firstKeyRaw.split (':')[1]);
    var lastKey = Number (lastKeyRaw.split (':')[1]);
  
  	ctatdebug ("lengthOnEditQueue ("+lastKey - firstKey + 1 +")");
...
function authorize()
{
	...
	var ui = SpreadsheetApp.getUi();
	var html = HtmlService.createHtmlOutput('Authorization complete<br><br>please refresh your browser and choose Start Tutor from the CTAT menu').setTitle("Confirmation").setWidth(400);
	ui.showSidebar(html);
}

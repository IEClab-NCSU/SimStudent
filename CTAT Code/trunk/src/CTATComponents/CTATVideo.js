/**-----------------------------------------------------------------------------
 $Author: mdb91 $
 $Date: 2017-04-18 18:00:10 -0500 (週二, 18 四月 2017) $
...
CTATVideo = function(aDescription,aX,aY,aWidth,aHeight)
{
	...
	this.init=function init()
	{
		...
		if (this.getDivWrap() 
		&&  $(this.getDivWrap()).attr('src')
		&&	!CTATConfiguration.get('previewMode'))
		{
			video.src = $(this.getDivWrap()).attr('src');
		}/*
		else (Broken link, and not sure why we'd want a default anyway)
		{
	...
	this.logVideoEvent=function logVideoEvent (eventName)
	{
		...
		if (video.duration!==undefined)
		{
			if (CTATCommShell.commShell!==null)
			{
				...
				CTATCommShell.commShell.processComponentAction (audioSAI,
																pointer.getTutorComponent(),
																true,
																pointer,
																"VIDEO_ACTION",
																"USER");
